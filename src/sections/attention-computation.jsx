    ...
    <Reveal when={sub >= 1}><Box color={C.purple} style={{ width: "100%" }}>
        ...
        <div style={{ marginTop: 10, display: "flex", flexDirection: "column", gap: 12 }}>
          {[
            ...
            { name: "W_K", c: C.orange, rows: [[-0.12, 0.16], [0.51, 0.37], [0.50, 0.22], [0.39, 0.53]] },
            { name: "W_V", c: C.green, rows: [[0.59, -0.49], [0.32, 0.64], [-0.21, 0.77], [0.24, -0.13]] },
          ].map(({ name, c, rows }) => (
            <div key={name} style={{ padding: "8px 10px", borderRadius: 8, background: `${c}06`, border: `1px solid ${c}12` }}>
              <T color={c} bold center size={16}>{name} (4×2)</T>
              <div style={{ marginTop: 4, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 2, maxWidth: 160, margin: "4px auto 0" }}>
                {rows.flat().map((v, i) => (
                  <div key={i} style={{ textAlign: "center", padding: "2px 4px", borderRadius: 3, background: `${c}08` }}>
        ...
        <T color={C.dim} size={16} style={{ marginTop: 8 }}>These numbers were nudged by training over millions of examples. Each matrix learned to extract a different "aspect" of meaning.</T>
      </Box></Reveal>

    <Reveal when={sub >= 2}><Box color={C.red} style={{ width: "100%" }}>
        <T color="#ef9a9a" bold center>Let's multiply "I"'s embedding by all three matrices:</T>
        <T color={C.dim} size={16} style={{ marginTop: 4 }}>"I" = [1.0, 0.5, -0.3, 0.8]. Each dim of the embedding multiplies the corresponding row of W, then we sum.</T>
        <div style={{ marginTop: 10, display: "flex", flexDirection: "column", gap: 10 }}>
          {[
            { name: "W_Q", c: C.blue, col1: [0.15, 0.12, 0.10, 0.78], col2: [-0.03, 0.78, 0.51, -0.01],
              calc1: "(1.0 x 0.15) + (0.5 x 0.12) + (-0.3 x 0.10) + (0.8 x 0.78)", r1: "0.8",
              calc2: "(1.0 x -0.03) + (0.5 x 0.78) + (-0.3 x 0.51) + (0.8 x -0.01)", r2: "0.2",
              result: "[0.8, 0.2]" },
            { name: "W_K", c: C.orange, col1: [-0.12, 0.51, 0.50, 0.39], col2: [0.16, 0.37, 0.22, 0.53],
              calc1: "(1.0 x -0.12) + (0.5 x 0.51) + (-0.3 x 0.50) + (0.8 x 0.39)", r1: "0.3",
              calc2: "(1.0 x 0.16) + (0.5 x 0.37) + (-0.3 x 0.22) + (0.8 x 0.53)", r2: "0.7",
              result: "[0.3, 0.7]" },
            { name: "W_V", c: C.green, col1: [0.59, 0.32, -0.21, 0.24], col2: [-0.49, 0.64, 0.77, -0.13],
              calc1: "(1.0 x 0.59) + (0.5 x 0.32) + (-0.3 x -0.21) + (0.8 x 0.24)", r1: "1.0",
              calc2: "(1.0 x -0.49) + (0.5 x 0.64) + (-0.3 x 0.77) + (0.8 x -0.13)", r2: "-0.5",
              result: "[1.0, -0.5]" },
          ].map(({ name, c, calc1, r1, calc2, r2, result }) => (
            <div key={name} style={{ padding: "8px 10px", borderRadius: 8, background: `${c}06`, border: `1px solid ${c}12` }}>
              <T color={c} bold size={16}>"I" x {name}:</T>
              <div style={{ marginTop: 4, display: "flex", flexDirection: "column", gap: 2, paddingLeft: 8 }}>
                <T color={C.dim} size={14}>dim 1: {calc1} = <strong style={{ color: c }}>{r1}</strong></T>
                <T color={C.dim} size={14}>dim 2: {calc2} = <strong style={{ color: c }}>{r2}</strong></T>
              </div>
              <T color={c} bold center size={16} style={{ marginTop: 4 }}>{name === "W_Q" ? "Q" : name === "W_K" ? "K" : "V"}_I = {result}</T>
            </div>
          ))}
        </div>
        <T color="#ef9a9a" size={16} style={{ marginTop: 8 }}>"love" and "cats" go through the exact same three matrices - different embeddings in, different Q/K/V out.</T>
      </Box></Reveal>
...
export const CausalMask = (ctx) => { const { sub, subBtnRipple, setSubBtnRipple, registerSubBtn, navigate } = ctx;
  ...
  const maskedScores = rawScores.map((row, i) =>
    row.map((v, j) => j <= i ? v : -Infinity)
  );
  ...
  const Cell = ({ val, color, bold, size = 16 }) => (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", padding: "5px 2px", borderRadius: 5, background: `${color}08`, minWidth: 48 }}>
  ...
  <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 16 }}>
    ...
    {sub >= 0 && (
      <Box color={C.red} style={{ width: "100%" }}>
        <T color="#ef9a9a" bold center size={20}>The problem: during generation, future words don't exist yet</T>
        <T color="#ef9a9a" style={{ marginTop: 8 }}>When the model generates text one token at a time, it has only produced the tokens so far. The next word hasn't been generated yet - it can't attend to something that doesn't exist.</T>
        <div style={{ marginTop: 12, padding: "12px", background: "rgba(0,0,0,0.3)", borderRadius: 10 }}>
          <T color={C.dim} size={14} center style={{ letterSpacing: 1.5, textTransform: "uppercase", marginBottom: 8 }}>generating "The cat sat on the mat"</T>
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            {[
              ...
              <div key={step} style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 10px", borderRadius: 6, background: `${wColors[step - 1]}06`, border: `1px solid ${wColors[step - 1]}12` }}>
                <span style={{ color: wColors[step - 1], fontWeight: 800, fontSize: 16, minWidth: 20 }}>{step}.</span>
                <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
                  {seen.map((w, i) => (
                    <span key={i} style={{ padding: "2px 8px", borderRadius: 4, background: `${C.green}15`, color: C.green, fontSize: 16, fontWeight: 600 }}>{w}</span>
                  ))}
                  <span style={{ color: C.dim, fontSize: 14, alignSelf: "center" }}>→</span>
                  <span style={{ padding: "2px 8px", borderRadius: 4, background: `${wColors[step - 1]}15`, color: wColors[step - 1], fontSize: 16, fontWeight: 700, border: `1px dashed ${wColors[step - 1]}40` }}>{gen}?</span>
                </div>
                <T color={C.dim} size={13} style={{ marginLeft: "auto" }}>{note}</T>
              </div>
            ))}
          </div>
        </div>
        <T color="#ef9a9a" style={{ marginTop: 10 }}>But during <strong>training</strong>, the model sees the entire sentence at once for efficiency. So we need to <strong>artificially hide</strong> future tokens - that's what the causal mask does.</T>
      </Box>
    )}

    {/* Sub 1: Who can look at whom - intuitive grid + score matrix */}
    <Reveal when={sub >= 1}><Box color={C.cyan} style={{ width: "100%" }}>
      <T color="#80deea" bold center size={20}>Who can look at whom?</T>
      <T color="#80deea" style={{ marginTop: 6 }}>Each row is a word asking "who can I attend to?" Each column is a word being looked at. A word can only see words generated <strong>before it</strong> (+ itself):</T>

      {/* Intuitive checkmark/cross grid */}
      <div style={{ marginTop: 12, overflowX: "auto" }}>
        <div style={{ display: "grid", gridTemplateColumns: `100px repeat(${words.length}, 1fr)`, gap: 4, alignItems: "center" }}>
          {/* Column headers */}
          <div style={{ padding: "4px", textAlign: "center" }}>
            <T color={C.dim} size={11}>row asks →</T>
            <T color={C.dim} size={11}>col answers ↓</T>
          </div>
          {words.map((w, i) => <T key={i} color={wColors[i]} size={15} bold center>{w}</T>)}

          {/* Grid rows with explanations */}
          {words.map((w, ri) => (
            <div key={ri} style={{ display: "contents" }}>
              <T color={wColors[ri]} size={14} bold>"{w}" asks →</T>
              {words.map((_, ci) => {
                const allowed = ci <= ri;
                return (
                  <div key={ci} style={{
                    ...
                    <span style={{ fontSize: 20, color: allowed ? C.green : C.red }}>{allowed ? "\u2713" : "\u2717"}</span>
                  </div>
                );
              })}
            </div>
      ...
      <div style={{ marginTop: 10, display: "flex", flexDirection: "column", gap: 4 }}>
        {[
          ...
          <div key={w} style={{ display: "flex", alignItems: "center", gap: 8, padding: "4px 10px", borderRadius: 6, background: `${c}06` }}>
            <span style={{ color: c, fontWeight: 700, fontSize: 16, minWidth: 36 }}>"{w}"</span>
            <T color={C.dim} size={14}>{why}</T>
          </div>
        ))}
      </div>

      <div style={{ marginTop: 10, padding: "10px 12px", borderRadius: 8, background: `${C.green}06`, border: `1px solid ${C.green}15` }}>
        <T color="#80e8a5" bold center>The diagonal = each word seeing itself (always allowed)</T>
        <T color="#80e8a5" center size={15} style={{ marginTop: 2 }}>Below diagonal = looking at earlier words (allowed - they already exist)</T>
        <T color="#ef9a9a" center size={15} style={{ marginTop: 2 }}>Above diagonal = looking at future words (forbidden - not generated yet)</T>
      </div>

      {/* Now the actual Q.K^T score matrix with same color coding */}
      <T color="#80deea" bold center size={18} style={{ marginTop: 14 }}>Now here is the actual Q·K<sup>T</sup> / √d<sub>k</sub> score matrix:</T>
      <div style={{ marginTop: 8, overflowX: "auto" }}>
        <div style={{ display: "grid", gridTemplateColumns: `70px repeat(${words.length}, 1fr)`, gap: 4, alignItems: "center" }}>
          <div></div>
          {words.map((w, i) => <T key={i} color={wColors[i]} size={14} bold center>{w}</T>)}
          {rawScores.map((row, ri) => (
            <div key={ri} style={{ display: "contents" }}>
              <T color={wColors[ri]} size={14} bold>"{words[ri]}" →</T>
              {row.map((v, ci) => <Cell key={ci} val={v.toFixed(1)} color={ci <= ri ? C.green : C.red} />)}
            </div>
          ))}
        </div>
      </div>
      <div style={{ marginTop: 8, display: "flex", gap: 12, justifyContent: "center" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 4 }}><div style={{ width: 12, height: 12, borderRadius: 3, background: `${C.green}30` }} /><T color={C.dim} size={13}>allowed (past + self) - keep these scores</T></div>
        <div style={{ display: "flex", alignItems: "center", gap: 4 }}><div style={{ width: 12, height: 12, borderRadius: 3, background: `${C.red}30` }} /><T color={C.dim} size={13}>future - must be blocked!</T></div>
      </div>
    </Box></Reveal>

    {/* Sub 2: The mask matrix */}
    <Reveal when={sub >= 2}><Box color={C.purple} style={{ width: "100%" }}>
      <T color="#b8a9ff" bold center size={20}>The mask: a triangle of negative infinity</T>
      <T color="#b8a9ff" style={{ marginTop: 6 }}>We add a mask matrix to the scores. The mask has <strong>0</strong> where a word is allowed to look, and <strong>-∞</strong> where it's forbidden:</T>
      <div style={{ marginTop: 12, overflowX: "auto" }}>
        <div style={{ display: "grid", gridTemplateColumns: `70px repeat(${words.length}, 1fr)`, gap: 4, alignItems: "center" }}>
          <div></div>
          {words.map((w, i) => <T key={i} color={wColors[i]} size={14} bold center>{w}</T>)}
          {words.map((w, ri) => (
            <div key={ri} style={{ display: "contents" }}>
              <T color={wColors[ri]} size={14} bold>"{w}" →</T>
              {words.map((_, ci) => {
                const allowed = ci <= ri;
                return <Cell key={ci} val={allowed ? "0" : "-\u221E"} color={allowed ? C.green : C.red} bold={!allowed} size={allowed ? 16 : 14} />;
              })}
            </div>
          ))}
        </div>
      </div>
      <T color="#b8a9ff" style={{ marginTop: 10 }}>Adding -∞ to a score makes it -∞. When softmax sees -∞, it outputs <strong>exactly 0</strong> - zero attention, as if that word doesn't exist. The lower triangle (0s) keeps scores unchanged.</T>
      <div style={{ marginTop: 8, padding: "10px 12px", borderRadius: 8, background: `${C.purple}06`, border: `1px solid ${C.purple}15` }}>
        <T color="#b8a9ff" size={16} center>masked_scores = scores + mask</T>
        <T color={C.dim} size={14} center style={{ marginTop: 2 }}>Where mask[i][j] = 0 if j ≤ i, else -∞</T>
      </div>
    </Box></Reveal>
    ...
    <Reveal when={sub >= 3}><Box color={C.orange} style={{ width: "100%" }}>
      ...
      <div style={{ marginTop: 4, overflowX: "auto" }}>
        <div style={{ display: "grid", gridTemplateColumns: `70px repeat(${words.length}, 1fr)`, gap: 4, alignItems: "center" }}>
          <div></div>
          {words.map((w, i) => <T key={i} color={wColors[i]} size={13} bold center>{w}</T>)}
          {maskedScores.map((row, ri) => (
            <div key={ri} style={{ display: "contents" }}>
              <T color={wColors[ri]} size={13} bold>"{words[ri]}" →</T>
              {row.map((v, ci) => <Cell key={ci} val={v === -Infinity ? "-\u221E" : v.toFixed(1)} color={v === -Infinity ? C.red : C.green} bold={v === -Infinity} size={14} />)}
            </div>
          ))}
        </div>
      </div>
    ...
    <Reveal when={sub >= 4}><Box color={C.green} style={{ width: "100%" }}>
      ...
      <T color={C.dim} size={12} center style={{ letterSpacing: 1.5, textTransform: "uppercase" }}>attention weights (probabilities)</T>
      <div style={{ marginTop: 4, overflowX: "auto" }}>
        <div style={{ display: "grid", gridTemplateColumns: `70px repeat(${words.length}, 1fr)`, gap: 4, alignItems: "center" }}>
          <div></div>
          {words.map((w, i) => <T key={i} color={wColors[i]} size={13} bold center>{w}</T>)}
          {maskedProbs.map((row, ri) => (
            <div key={ri} style={{ display: "contents" }}>
              ...
              {row.map((v, ci) => {
                ...
                return <Cell key={ci} val={v.toFixed(2)} color={isZero ? C.red : C.green} bold={!isZero && v > 0.35} size={14} />;
              })}
            </div>
          ))}
        </div>
      ...
      <div style={{ marginTop: 10, padding: "10px", borderRadius: 8, background: `${C.green}06`, border: `1px solid ${C.green}15` }}>
        <T color="#80e8a5" bold center>Every -∞ became exactly 0.00 after softmax!</T>
        <T color="#80e8a5" center size={16} style={{ marginTop: 2 }}>"The" puts 100% attention on itself. "cat" splits between "The" and itself. Each row still sums to 1.0 - but only across the visible tokens.</T>
      </div>
    </Box></Reveal>
    ...
    <Reveal when={sub >= 5}><Box color={C.yellow} style={{ width: "100%" }}>
      ...
      <div style={{ marginTop: 12, display: "flex", gap: 12, flexWrap: "wrap", justifyContent: "center" }}>
        ...
        <div style={{ flex: 1, minWidth: 180 }}>
          <T color={C.cyan} bold center size={16}>No mask (bidirectional)</T>
          <T color={C.dim} center size={13}>every word sees every word</T>
          <div style={{ marginTop: 6, display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 3 }}>
            {words.map((_, ri) => words.map((_, ci) => (
              <div key={`b${ri}${ci}`} style={{ height: 28, borderRadius: 4, background: `${C.cyan}25`, display: "flex", alignItems: "center", justifyContent: "center" }}>
                <span style={{ color: C.cyan, fontSize: 11, fontWeight: 600 }}>{fullProbs[ri][ci].toFixed(2)}</span>
              </div>
            )))}
          </div>
          <T color={C.dim} center size={12} style={{ marginTop: 4 }}>Full matrix - all cells active</T>
        </div>

        {/* Causal grid */}
        <div style={{ flex: 1, minWidth: 180 }}>
          <T color={C.orange} bold center size={16}>Causal mask</T>
          <T color={C.dim} center size={13}>each word sees only past + self</T>
          <div style={{ marginTop: 6, display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 3 }}>
            {words.map((_, ri) => words.map((_, ci) => {
              const allowed = ci <= ri;
              return (
                <div key={`c${ri}${ci}`} style={{ height: 28, borderRadius: 4, background: allowed ? `${C.orange}25` : "rgba(255,255,255,0.03)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                  <span style={{ color: allowed ? C.orange : "rgba(255,255,255,0.12)", fontSize: 11, fontWeight: allowed ? 600 : 400 }}>{allowed ? maskedProbs[ri][ci].toFixed(2) : "0"}</span>
                </div>
              );
            }))}
          </div>
          <T color={C.dim} center size={12} style={{ marginTop: 4 }}>Lower triangle only - upper triangle is dark</T>
        </div>
      </div>
    ...
    <Reveal when={sub >= 7}><Box color={C.green} style={{ width: "100%" }}>
      ...
      <div style={{ marginTop: 12, display: "flex", flexDirection: "column", gap: 10 }}>
        {/* Encoder-only */}
        <div style={{ padding: "14px", borderRadius: 10, background: `${C.cyan}06`, border: `1px solid ${C.cyan}15`, display: "flex", flexDirection: "column", alignItems: "center" }}>
          <div style={{ padding: "3px 10px", borderRadius: 5, background: `${C.cyan}20`, border: `1px solid ${C.cyan}40` }}>
            <T color={C.cyan} bold size={14} center>ENCODER-ONLY</T>
          </div>
          <T color={C.dim} size={14} center style={{ marginTop: 4, marginBottom: 8 }}>BERT, RoBERTa</T>
          ...
          <div style={{ marginTop: 8, textAlign: "center" }}>
            <T color="#80deea" size={16}><strong>No mask</strong> - every word sees every other word (bidirectional).</T>
            <T color="#80deea" size={16} style={{ marginTop: 4 }}><strong>Why?</strong> The encoder's job is to <strong>understand</strong> the full input. When classifying "The bank by the river", "bank" NEEDS to see "river" (which comes after it) to know it means riverbank, not a financial bank. Hiding future words would cripple understanding.</T>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        ...
        <div style={{ padding: "14px", borderRadius: 10, background: `${C.orange}06`, border: `1px solid ${C.orange}15`, display: "flex", flexDirection: "column", alignItems: "center" }}>
          <div style={{ padding: "3px 10px", borderRadius: 5, background: `${C.orange}20`, border: `1px solid ${C.orange}40` }}>
            <T color={C.orange} bold size={14} center>DECODER-ONLY</T>
          </div>
          <T color={C.dim} size={14} center style={{ marginTop: 4, marginBottom: 8 }}>GPT, Claude, LLaMA</T>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 24px)", gap: 3 }}>
            {Array.from({ length: 16 }).map((_, i) => {
              const r = Math.floor(i / 4), col = i % 4;
              const on = col <= r;
              return <div key={i} style={{ width: 24, height: 24, borderRadius: 4, background: on ? `${C.orange}30` : "rgba(255,255,255,0.04)" }} />;
            })}
          </div>
          <T color={C.dim} center size={12} style={{ marginTop: 4 }}>Lower triangle only - upper triangle masked</T>
          <div style={{ marginTop: 8, textAlign: "center" }}>
            <T color="#ffb74d" size={16}><strong>Causal mask</strong> - each word only sees past words + itself.</T>
            <T color="#ffb74d" size={16} style={{ marginTop: 4 }}><strong>Why?</strong> The decoder <strong>generates</strong> text one token at a time. At generation step 3, token 4 doesn't exist yet - attending to it would be looking at something that isn't there. During training, the full sentence is fed at once for speed, so we use the mask to simulate the generation condition. Without it, the model would "cheat" by seeing the answer, then fail at actual generation.</T>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        ...
        <div style={{ padding: "14px", borderRadius: 10, background: `${C.purple}06`, border: `1px solid ${C.purple}15`, display: "flex", flexDirection: "column", alignItems: "center" }}>
          <div style={{ padding: "3px 10px", borderRadius: 5, background: `${C.purple}20`, border: `1px solid ${C.purple}40` }}>
            <T color={C.purple} bold size={14} center>ENCODER-DECODER</T>
          </div>
          <T color={C.dim} size={14} center style={{ marginTop: 4, marginBottom: 8 }}>Original Transformer, T5, BART</T>
          <div style={{ display: "flex", gap: 10, alignItems: "flex-end" }}>
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
              <T color={C.cyan} size={10} center>encoder</T>
            ...
            <T color={C.dim} size={16} style={{ paddingBottom: 16 }}>+</T>
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
              <T color={C.orange} size={10} center>decoder</T>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 20px)", gap: 2 }}>
                {Array.from({ length: 16 }).map((_, i) => {
                  const r = Math.floor(i / 4), col = i % 4;
                  const on = col <= r;
                  return <div key={i} style={{ width: 20, height: 20, borderRadius: 3, background: on ? `${C.orange}30` : "rgba(255,255,255,0.04)" }} />;
                })}
              </div>
            </div>
          </div>
          <T color={C.dim} center size={12} style={{ marginTop: 4 }}>Encoder: full matrix + Decoder: lower triangle</T>
          <div style={{ marginTop: 8, textAlign: "center" }}>
            <T color="#b8a9ff" size={16}><strong>Both</strong> - encoder uses no mask, decoder uses causal mask, plus cross-attention connecting them.</T>
            <T color="#b8a9ff" size={16} style={{ marginTop: 4 }}><strong>Why?</strong> The input (e.g., English sentence) already exists fully - the encoder should see all of it. But the output (e.g., French translation) is generated word by word - the decoder must not peek ahead. Cross-attention lets the decoder look at the encoder's full understanding while still generating left-to-right.</T>
            <T color={C.dim} size={14} style={{ marginTop: 4 }}>Used for: translation, summarization, any "input → different output" task.</T>
          </div>
        </div>
      </div>

      <div style={{ marginTop: 12, padding: "10px 12px", borderRadius: 8, background: `${C.yellow}06`, border: `1px solid ${C.yellow}15` }}>
        <T color={C.yellow} bold center>The core rule is simple</T>
        <T color="#ffe082" size={16} style={{ marginTop: 4 }}>If the text <strong>already exists</strong> (input you're reading) → no mask needed, let every word see everything for maximum understanding.</T>
        <T color="#ffe082" size={16} style={{ marginTop: 2 }}>If the text is <strong>being generated</strong> (output you're writing) → causal mask required, because future words don't exist yet.</T>
      </div>
    </Box></Reveal>
...
export const CrossAttention = (ctx) => { const { sub, subBtnRipple, setSubBtnRipple, registerSubBtn, navigate } = ctx;
  ...
  const Cell = ({ val, color, bold, size = 15 }) => (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", padding: "5px 2px", borderRadius: 5, background: `${color}08`, minWidth: 52 }}>
      <span style={{ fontFamily: "monospace", fontSize: size, color, fontWeight: bold ? 700 : 400 }}>{val}</span>
    </div>
  );
  ...
  <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 16 }}>
    ...
    {sub >= 0 && (
      <Box color={C.cyan} style={{ width: "100%" }}>
        <T color="#80deea" bold center size={20}>The setup: translating "I love cats" to French</T>
        <T color="#80deea" style={{ marginTop: 8 }}>The encoder has processed the English sentence. Each word is now a rich contextual vector - "love" doesn't just mean "love" anymore, it means "love in the context of I-love-cats".</T>

        <div style={{ marginTop: 12, display: "flex", flexDirection: "column", gap: 10 }}>
          {/* Encoder output */}
          <div style={{ padding: "10px 12px", borderRadius: 8, background: `${C.cyan}06`, border: `1px solid ${C.cyan}15` }}>
            <T color={C.cyan} bold center size={16}>Encoder output (English - fully processed)</T>
            <div style={{ display: "flex", gap: 8, justifyContent: "center", marginTop: 8 }}>
              {encWords.map((w, i) => (
                <div key={i} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
                  <span style={{ padding: "4px 14px", borderRadius: 6, background: `${encColors[i]}18`, border: `1px solid ${encColors[i]}35`, color: encColors[i], fontWeight: 700, fontSize: 18 }}>{w}</span>
                  <span style={{ color: C.dim, fontSize: 11 }}>768 dims</span>
                </div>
              ))}
            </div>
            <T color={C.dim} center size={13} style={{ marginTop: 6 }}>These vectors are the encoder's final output - rich with context</T>
          </div>

          {/* Arrow */}
          <T color={C.dim} size={20} center>↓ feeds into decoder's cross-attention</T>

          {/* Decoder in progress */}
          <div style={{ padding: "10px 12px", borderRadius: 8, background: `${C.orange}06`, border: `1px solid ${C.orange}15` }}>
            <T color={C.orange} bold center size={16}>Decoder (French - generating word by word)</T>
            <div style={{ display: "flex", gap: 8, justifyContent: "center", marginTop: 8 }}>
              {decWords.map((w, i) => (
                <div key={i} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
                  <span style={{ padding: "4px 14px", borderRadius: 6, background: `${decColors[i]}18`, border: `1px solid ${decColors[i]}35`, color: decColors[i], fontWeight: 700, fontSize: 18 }}>{w}</span>
                  <span style={{ color: C.dim, fontSize: 11 }}>{i < 3 ? "generated" : "next?"}</span>
                </div>
              ))}
            </div>
            <T color={C.dim} center size={13} style={{ marginTop: 6 }}>The decoder needs to look at the English to know what to generate next</T>
          </div>
        </div>

        <T color="#80deea" style={{ marginTop: 10 }}>The decoder's self-attention lets French words look at other French words. But how does the decoder look at the <strong>English</strong> words? That's what cross-attention does.</T>
      </Box>
    )}

    {/* Sub 1: Q from decoder, K and V from encoder */}
    <Reveal when={sub >= 1}><Box color={C.yellow} style={{ width: "100%" }}>
      <T color={C.yellow} bold center size={20}>The key idea: Q from decoder, K and V from encoder</T>
      <T color="#ffe082" style={{ marginTop: 6 }}>In self-attention, Q, K, and V all come from the same sentence. In cross-attention, they come from <strong>different</strong> sources:</T>

      <div style={{ marginTop: 12, display: "flex", flexDirection: "column", gap: 8 }}>
        {[
          { role: "Query (Q)", source: "Decoder (French)", why: "The decoder is asking: \"what English word should I pay attention to right now?\"", color: C.orange, icon: "Q" },
          { role: "Key (K)", source: "Encoder (English)", why: "The encoder's words offer their 'labels' for matching - \"I'm the subject\", \"I'm the verb\"", color: C.cyan, icon: "K" },
          { role: "Value (V)", source: "Encoder (English)", why: "The encoder's words offer their actual content - the rich contextual meaning to borrow from", color: C.cyan, icon: "V" },
        ].map(({ role, source, why, color, icon }) => (
          <div key={role} style={{ padding: "10px 12px", borderRadius: 8, background: `${color}06`, border: `1px solid ${color}15`, display: "flex", gap: 10, alignItems: "flex-start" }}>
            <div style={{ padding: "6px 10px", borderRadius: 6, background: `${color}20`, border: `1px solid ${color}40`, flexShrink: 0 }}>
              <span style={{ color, fontWeight: 800, fontSize: 20 }}>{icon}</span>
            </div>
            <div>
              <T color={color} bold size={16}>{role} ← {source}</T>
              <T color={C.dim} size={15} style={{ marginTop: 2 }}>{why}</T>
            </div>
          </div>
        ))}
      </div>

      <div style={{ marginTop: 10, padding: "10px 12px", borderRadius: 8, background: `${C.yellow}06`, border: `1px solid ${C.yellow}15` }}>
        <T color={C.yellow} bold center>Same formula: Attention(Q, K, V) = softmax( Q·K<sup>T</sup> / √d<sub>k</sub> ) · V</T>
        <T color="#ffe082" center size={15} style={{ marginTop: 2 }}>The math is identical to self-attention. Only the source of Q, K, V changes.</T>
      </div>
    </Box></Reveal>

    {/* Sub 2: Concrete computation with scores */}
    <Reveal when={sub >= 2}><Box color={C.green} style={{ width: "100%" }}>
      <T color="#80e8a5" bold center size={20}>Tracing cross-attention: French Q meets English K</T>
      <T color="#80e8a5" style={{ marginTop: 6 }}>Each French word's Query asks "which English word is most relevant to me?" The score matrix is <strong>not square</strong> - it's 4 French rows x 3 English columns:</T>

      {/* Score matrix */}
      <T color={C.dim} size={12} center style={{ marginTop: 12, letterSpacing: 1.5, textTransform: "uppercase" }}>cross-attention scores (Q·K<sup>T</sup> / √d<sub>k</sub>)</T>
      <div style={{ marginTop: 4, overflowX: "auto" }}>
        <div style={{ display: "grid", gridTemplateColumns: `80px repeat(${encWords.length}, 1fr)`, gap: 4, alignItems: "center" }}>
          <div style={{ textAlign: "right", paddingRight: 4 }}>
            <T color={C.dim} size={11}>decoder Q ↓</T>
            <T color={C.dim} size={11}>encoder K →</T>
          </div>
          {encWords.map((w, i) => <T key={i} color={encColors[i]} size={15} bold center>{w}</T>)}
          {crossScores.map((row, ri) => (
            <div key={ri} style={{ display: "contents" }}>
              <T color={decColors[ri]} size={14} bold style={{ textAlign: "right", paddingRight: 4 }}>"{decWords[ri]}" →</T>
              {row.map((v, ci) => {
      ...
      {/* Probability matrix */}
      <T color={C.dim} size={12} center style={{ letterSpacing: 1.5, textTransform: "uppercase" }}>attention weights</T>
      <div style={{ marginTop: 4, overflowX: "auto" }}>
        <div style={{ display: "grid", gridTemplateColumns: `80px repeat(${encWords.length}, 1fr)`, gap: 4, alignItems: "center" }}>
          <div></div>
          {encWords.map((w, i) => <T key={i} color={encColors[i]} size={14} bold center>{w}</T>)}
      ...
      <div style={{ marginTop: 10, display: "flex", flexDirection: "column", gap: 4 }}>
        {[
          { fr: "J'", en: "I", pct: crossProbs[0][0], c: C.orange, note: "\"J'\" (I) focuses on \"I\" - direct translation" },
          { fr: "aime", en: "love", pct: crossProbs[1][1], c: C.yellow, note: "\"aime\" (love) focuses on \"love\" - the verb it's translating" },
          { fr: "les", en: "cats", pct: crossProbs[2][2], c: C.orange, note: "\"les\" (the) focuses on \"cats\" - it's the article FOR cats" },
          { fr: "chats", en: "cats", pct: crossProbs[3][2], c: C.yellow, note: "\"chats\" (cats) focuses on \"cats\" - direct translation" },
        ].map(({ fr, en, pct, c, note }) => (
          <div key={fr} style={{ display: "flex", alignItems: "center", gap: 8, padding: "4px 10px", borderRadius: 6, background: `${c}06` }}>
            <span style={{ color: c, fontWeight: 700, fontSize: 16, minWidth: 50 }}>"{fr}"</span>
            <span style={{ color: C.dim, fontSize: 14 }}>→</span>
            <span style={{ color: C.green, fontWeight: 700, fontSize: 16 }}>{(pct * 100).toFixed(0)}% on "{en}"</span>
            <T color={C.dim} size={13} style={{ marginLeft: "auto" }}>{note}</T>
          </div>
        ))}
      </div>
    ...
    <Reveal when={sub >= 3}><Box color={C.purple} style={{ width: "100%" }}>
      <T color="#b8a9ff" bold center size={20}>Self-attention vs Cross-attention</T>
      <T color="#b8a9ff" style={{ marginTop: 6 }}>Same formula, different wiring:</T>

      <div style={{ marginTop: 12, display: "flex", gap: 12, flexWrap: "wrap", justifyContent: "center" }}>
        {/* Self-attention */}
        <div style={{ flex: 1, minWidth: 180, padding: "12px", borderRadius: 10, background: `${C.cyan}06`, border: `1px solid ${C.cyan}15`, display: "flex", flexDirection: "column", alignItems: "center" }}>
          <div style={{ padding: "3px 10px", borderRadius: 5, background: `${C.cyan}20`, border: `1px solid ${C.cyan}40` }}>
            <T color={C.cyan} bold size={14} center>SELF-ATTENTION</T>
          </div>
          <div style={{ marginTop: 8, padding: "8px 12px", borderRadius: 6, background: `${C.cyan}10` }}>
            <T color={C.cyan} bold center size={14}>same sentence</T>
          </div>
          <div style={{ marginTop: 6, display: "flex", gap: 4 }}>
            {["Q", "K", "V"].map(r => (
              <div key={r} style={{ padding: "4px 10px", borderRadius: 5, background: `${C.cyan}15` }}>
                <T color={C.cyan} bold size={16} center>{r}</T>
              </div>
            ))}
          </div>
          <T color={C.dim} size={13} center style={{ marginTop: 6 }}>Q, K, V all from the same sentence. Words attend to each other within one language.</T>
          <T color={C.dim} size={13} center style={{ marginTop: 4 }}>Score matrix is <strong style={{ color: C.cyan }}>square</strong> (N x N)</T>
        </div>

        {/* Cross-attention */}
        <div style={{ flex: 1, minWidth: 180, padding: "12px", borderRadius: 10, background: `${C.purple}06`, border: `1px solid ${C.purple}15`, display: "flex", flexDirection: "column", alignItems: "center" }}>
          <div style={{ padding: "3px 10px", borderRadius: 5, background: `${C.purple}20`, border: `1px solid ${C.purple}40` }}>
            <T color={C.purple} bold size={14} center>CROSS-ATTENTION</T>
          </div>
          <div style={{ marginTop: 8, display: "flex", gap: 6 }}>
            <div style={{ padding: "8px 12px", borderRadius: 6, background: `${C.orange}10` }}>
              <T color={C.orange} bold center size={14}>decoder</T>
            </div>
            <div style={{ padding: "8px 12px", borderRadius: 6, background: `${C.cyan}10` }}>
              <T color={C.cyan} bold center size={14}>encoder</T>
            </div>
          </div>
          <div style={{ marginTop: 6, display: "flex", gap: 4 }}>
            <div style={{ padding: "4px 10px", borderRadius: 5, background: `${C.orange}15` }}>
              <T color={C.orange} bold size={16} center>Q</T>
            </div>
            <div style={{ padding: "4px 10px", borderRadius: 5, background: `${C.cyan}15` }}>
              <T color={C.cyan} bold size={16} center>K</T>
            </div>
            <div style={{ padding: "4px 10px", borderRadius: 5, background: `${C.cyan}15` }}>
              <T color={C.cyan} bold size={16} center>V</T>
            </div>
          </div>
          <T color={C.dim} size={13} center style={{ marginTop: 6 }}>Q from decoder, K and V from encoder. Decoder attends to encoder's output.</T>
          <T color={C.dim} size={13} center style={{ marginTop: 4 }}>Score matrix is <strong style={{ color: C.purple }}>rectangular</strong> (M x N)</T>
        </div>
      </div>

      <div style={{ marginTop: 10, padding: "10px 12px", borderRadius: 8, background: `${C.yellow}06`, border: `1px solid ${C.yellow}15` }}>
        <T color={C.yellow} bold center>Each decoder layer has BOTH types of attention:</T>
        <T color="#ffe082" size={16} style={{ marginTop: 4 }}>1. <strong>Masked self-attention</strong> - French words look at other French words (with causal mask)</T>
        <T color="#ffe082" size={16} style={{ marginTop: 2 }}>2. <strong>Cross-attention</strong> - French words look at English words (no mask - the full English sentence already exists)</T>
      </div>
    </Box></Reveal>
    ...
    <Reveal when={sub >= 4}><Box color={C.red} style={{ width: "100%" }}>
      <T color="#ef9a9a" bold center size={20}>Critical detail: the encoder runs ONCE</T>
      <T color="#ef9a9a" style={{ marginTop: 8 }}>A common misconception is that encoder layer 1 feeds decoder layer 1, encoder layer 2 feeds decoder layer 2, etc. That's <strong>not</strong> how it works.</T>

      {/* Misconception vs Reality side by side - centered */}
      <div style={{ marginTop: 14, display: "flex", gap: 16, justifyContent: "center", alignItems: "stretch", flexWrap: "wrap" }}>
        {/* WRONG: layer-to-layer */}
        <div style={{ width: 200, padding: 16, borderRadius: 10, background: `${C.red}08`, border: `1px solid ${C.red}25`, position: "relative", display: "flex", flexDirection: "column", alignItems: "center" }}>
          <T color={C.red} bold center size={16}>WRONG</T>
          <T color={C.dim} center size={12} style={{ marginTop: 4 }}>Layer-to-layer mapping</T>
          <div style={{ marginTop: 12, display: "flex", flexDirection: "column", gap: 6 }}>
            {[6, 5, 4, 3, 2, 1].map(n => (
              <div key={n} style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
                <span style={{ fontSize: 13, color: C.cyan, padding: "3px 8px", borderRadius: 4, background: `${C.cyan}12`, fontWeight: 600 }}>E{n}</span>
                <span style={{ fontSize: 13, color: C.dim }}>→</span>
                <span style={{ fontSize: 13, color: C.orange, padding: "3px 8px", borderRadius: 4, background: `${C.orange}12`, fontWeight: 600 }}>D{n}</span>
              </div>
            ))}
          </div>
          {/* X overlay */}
        ...
        <div style={{ width: 220, padding: 16, borderRadius: 10, background: `${C.green}08`, border: `1px solid ${C.green}25`, display: "flex", flexDirection: "column", alignItems: "center" }}>
          <T color={C.green} bold center size={16}>CORRECT</T>
          <T color={C.dim} center size={12} style={{ marginTop: 4 }}>Only the final layer matters</T>
          <div style={{ marginTop: 12, display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
            <span style={{ fontSize: 13, color: C.cyan, padding: "4px 12px", borderRadius: 5, background: `${C.cyan}15`, border: `1px solid ${C.cyan}30`, fontWeight: 600 }}>Encoder Layer 6 (final)</span>
            <span style={{ fontSize: 16, color: C.green, fontWeight: 700 }}>↓</span>
            <span style={{ fontSize: 13, color: C.green, fontWeight: 700, padding: "4px 12px", borderRadius: 5, background: `${C.green}15`, border: `1px solid ${C.green}35` }}>K, V (computed once)</span>
            <div style={{ display: "flex", gap: 4, margin: "4px 0" }}>
              {[1, 2, 3, 4, 5, 6].map(n => <span key={n} style={{ fontSize: 14, color: `${C.green}70` }}>↓</span>)}
            </div>
            {[1, 2, 3, 4, 5, 6].map(n => (
              <span key={n} style={{ fontSize: 12, color: C.orange, padding: "2px 10px", borderRadius: 4, background: `${C.orange}08`, border: `1px solid ${C.orange}12` }}>Decoder Layer {n}</span>
            ))}
          </div>
        </div>
      </div>
      ...
      <div style={{ marginTop: 14, display: "flex", flexDirection: "column", gap: 6 }}>
        <div style={{ padding: "8px 12px", borderRadius: 8, background: `${C.cyan}06`, border: `1px solid ${C.cyan}15` }}>
          <T color="#80deea" size={16}><strong>Encoder:</strong> runs all 6 layers once on "I love cats". The output of layer 6 (the final layer) is the encoder's output - 3 contextual vectors, one per English word.</T>
        </div>
        <div style={{ padding: "8px 12px", borderRadius: 8, background: `${C.green}06`, border: `1px solid ${C.green}15` }}>
          <T color="#80e8a5" size={16}><strong>K and V:</strong> computed once from the encoder's final output using W<sub>K</sub> and W<sub>V</sub> weight matrices. These are then cached and reused.</T>
        </div>
        <div style={{ padding: "8px 12px", borderRadius: 8, background: `${C.orange}06`, border: `1px solid ${C.orange}15` }}>
          <T color="#ffb74d" size={16}><strong>Decoder layers 1-6:</strong> each layer computes its own Q (because the decoder's representation changes layer by layer as it gets refined). But K and V are always the same - the encoder's final output.</T>
        </div>
      </div>
    </Box></Reveal>
    ...
    <Reveal when={sub >= 5}><Box color={C.cyan} style={{ width: "100%" }}>
      <T color="#80deea" bold center size={20}>Complete data flow with dimensions</T>
      <T color={C.dim} center size={14} style={{ marginTop: 4 }}>d_model = 512, d_k = 64, 8 heads</T>

      <div style={{ marginTop: 14, padding: 16, background: "rgba(0,0,0,0.35)", borderRadius: 12, border: `1px solid ${C.border}`, display: "flex", justifyContent: "center" }}>
        <svg viewBox="0 0 540 370" style={{ width: "100%", maxWidth: 560 }}>
          {/* Encoder stack (left) - container centered at x=80 */}
          <rect x="15" y="10" width="130" height="220" rx="8" fill={`${C.cyan}08`} stroke={C.cyan} strokeOpacity="0.25" />
          <text x="80" y="30" fill={C.cyan} fontSize="12" fontWeight="700" textAnchor="middle">ENCODER</text>
          {[6, 5, 4, 3, 2, 1].map((n, i) => (
            <g key={n}>
              <rect x="30" y={40 + i * 27} width="100" height="22" rx="4"
                fill={n === 6 ? `${C.cyan}20` : `${C.cyan}08`}
                stroke={n === 6 ? C.cyan : `${C.cyan}30`} strokeOpacity={n === 6 ? 0.6 : 0.2} />
              <text x="80" y={55 + i * 27} fill={n === 6 ? C.cyan : C.dim} fontSize="10" textAnchor="middle">{n === 6 ? "Layer 6 (final)" : `Layer ${n}`}</text>
            </g>
          ))}
          <text x="80" y="242" fill={C.dim} fontSize="10" textAnchor="middle">"I love cats"</text>
          <text x="80" y="256" fill={C.dim} fontSize="9" textAnchor="middle">[3 x 512]</text>

          {/* Arrow from encoder final to projection box */}
          <line x1="145" y1="52" x2="178" y2="52" stroke={C.cyan} strokeWidth="2" strokeOpacity="0.6" markerEnd="url(#arrowCyanCA)" />

          {/* Projection box: W_K, W_V - wider for dimension text */}
          <rect x="182" y="12" width="166" height="90" rx="8" fill={`${C.green}10`} stroke={C.green} strokeOpacity="0.4" strokeWidth="1.5" />
          <text x="265" y="30" fill={C.green} fontSize="11" fontWeight="700" textAnchor="middle">Projection (once)</text>
          <text x="265" y="47" fill="#80e8a5" fontSize="10" textAnchor="middle">K = encoder_out · W_K</text>
          <text x="265" y="63" fill="#80e8a5" fontSize="10" textAnchor="middle">V = encoder_out · W_V</text>
          <text x="265" y="82" fill={C.dim} fontSize="9" textAnchor="middle">[3 x 512] · [512 x 64] = [3 x 64]</text>
          <text x="265" y="96" fill={C.dim} fontSize="8" textAnchor="middle">(per head)</text>

          {/* Arrow from projection to K,V cache */}
          <line x1="265" y1="102" x2="265" y2="120" stroke={C.green} strokeWidth="1.5" strokeOpacity="0.5" markerEnd="url(#arrowGreenCA)" />

          {/* K,V cache box */}
          <rect x="205" y="122" width="120" height="42" rx="6" fill={`${C.green}15`} stroke={C.green} strokeOpacity="0.5" strokeWidth="2" strokeDasharray="5,3" />
          <text x="265" y="141" fill={C.green} fontSize="11" fontWeight="700" textAnchor="middle">K, V CACHE</text>
          <text x="265" y="157" fill={C.dim} fontSize="9" textAnchor="middle">K: [3 x 64]  V: [3 x 64]</text>

          {/* Fan-out arrows from K,V cache to all decoder layers */}
          {[0, 1, 2, 3, 4, 5].map(i => (
            <line key={i} x1="325" y1={138 + i * 3} x2="360" y2={55 + i * 50} stroke={C.green} strokeWidth="1.2" strokeOpacity={0.3} strokeDasharray="4,3" />
          ))}

          {/* Decoder stack (right) - container centered at x=445 */}
          <rect x="360" y="10" width="170" height="340" rx="8" fill={`${C.orange}06`} stroke={C.orange} strokeOpacity="0.2" />
          <text x="445" y="30" fill={C.orange} fontSize="12" fontWeight="700" textAnchor="middle">DECODER</text>

          {[6, 5, 4, 3, 2, 1].map((n, i) => (
            <g key={n}>
              <rect x="372" y={38 + i * 50} width="146" height="40" rx="5"
                fill={`${C.orange}08`} stroke={C.orange} strokeOpacity="0.2" />
              <text x="392" y={55 + i * 50} fill={C.dim} fontSize="10" textAnchor="start">Layer {n}</text>
              {/* Q badge */}
              <rect x="438" y={44 + i * 50} width="18" height="16" rx="3" fill={`${C.orange}25`} />
              <text x="447" y={56 + i * 50} fill={C.orange} fontSize="10" fontWeight="700" textAnchor="middle">Q</text>
              <text x="460" y={56 + i * 50} fill={C.dim} fontSize="9" textAnchor="start">changes</text>
              {/* K,V badge */}
              <rect x="438" y={62 + i * 50} width="30" height="14" rx="3" fill={`${C.green}18`} />
              <text x="453" y={73 + i * 50} fill={C.green} fontSize="9" fontWeight="600" textAnchor="middle">K,V</text>
              <text x="472" y={73 + i * 50} fill={C.dim} fontSize="9" textAnchor="start">same</text>
            </g>
          ))}

          <text x="445" y="362" fill={C.dim} fontSize="10" textAnchor="middle">"J' aime les chats" [4 x 512]</text>
    ...
    <Reveal when={sub >= 6}><Box color={C.purple} style={{ width: "100%" }}>
      <T color="#b8a9ff" bold center size={20}>What happens at each decoder layer</T>
      <T color="#b8a9ff" size={16} style={{ marginTop: 6 }}>Every decoder layer performs cross-attention with the <strong>exact same K and V</strong>, but a <strong>different Q</strong>:</T>
      <div style={{ marginTop: 10, fontFamily: "monospace", padding: 12, borderRadius: 8, background: "rgba(0,0,0,0.3)", border: `1px solid ${C.purple}15` }}>
        {[1, 2, 6].map((n, i) => (
          <div key={n} style={{ marginTop: i > 0 ? 6 : 0, display: "flex", flexWrap: "wrap", gap: 4, alignItems: "center" }}>
            <span style={{ color: C.orange, fontSize: 14 }}>Layer {n}:</span>
            <span style={{ color: C.dim, fontSize: 13 }}>Attn(</span>
            <span style={{ color: C.orange, fontSize: 14, fontWeight: 700 }}>Q<sub style={{ fontSize: 10 }}>{n}</sub></span>
            ...
            <span style={{ color: C.orange, fontSize: 14, fontWeight: 700 }}>Q<sub style={{ fontSize: 10 }}>{n}</sub></span>
            <span style={{ color: C.dim, fontSize: 13 }}> · </span>
            <span style={{ color: C.green, fontSize: 14, fontWeight: 700 }}>K<sup style={{ fontSize: 10 }}>T</sup></span>
            <span style={{ color: C.dim, fontSize: 13 }}> / √64) · </span>
            <span style={{ color: C.green, fontSize: 14, fontWeight: 700 }}>V</span>
            {n === 2 && <span style={{ color: C.dim, fontSize: 12, marginLeft: 8 }}>... same K, V</span>}
            {i === 1 && <div style={{ width: "100%", textAlign: "center", color: C.dim, fontSize: 12, margin: "2px 0" }}>⋮</div>}
          </div>
        ))}
      </div>
      <T color={C.dim} size={14} style={{ marginTop: 8 }}>Q<sub>1</sub> comes from the decoder's layer 1 output. Q<sub>6</sub> comes from layer 6's output. The decoder's representation evolves layer by layer - Q captures "what I'm looking for" at each stage of refinement. K and V capture "what the encoder said" - and that never changes.</T>

      {/* Dimension walkthrough */}
      <div style={{ marginTop: 14, display: "flex", flexDirection: "column", gap: 6 }}>
        <T color={C.yellow} bold center size={17}>Dimension walkthrough (d_model = 512, heads = 8, d_k = 64)</T>
        {[
          { label: "Encoder output", dims: "[3 x 512]", detail: "3 English tokens, each 512 dims", color: C.cyan },
          { label: "W_K projection", dims: "[512 x 64]", detail: "per head, learned during training", color: C.green },
          ...
          { label: "Decoder layer n", dims: "[4 x 512]", detail: "4 French tokens, each 512 dims", color: C.orange },
          { label: "Q_n (per layer)", dims: "[4 x 64]", detail: "4 queries, changes each layer", color: C.orange },
          { label: "Q_n · K^T scores", dims: "[4 x 3]", detail: "every French word scores every English word", color: C.yellow },
        ].map((row, i) => (
          <div key={i} style={{ display: "flex", gap: 8, alignItems: "center", padding: "5px 10px", borderRadius: 6, background: `${row.color}06`, border: `1px solid ${row.color}12`, flexWrap: "wrap" }}>
            <span style={{ color: row.color, fontWeight: 700, fontSize: 14, minWidth: 140 }}>{row.label}</span>
            <span style={{ fontFamily: "monospace", color: row.color, fontSize: 15, fontWeight: 700, minWidth: 70 }}>{row.dims}</span>
      ...
      <div style={{ marginTop: 14, display: "flex", gap: 10, flexWrap: "wrap" }}>
        <div style={{ flex: "1 1 200px", padding: "10px 12px", borderRadius: 8, background: `${C.yellow}06`, border: `1px solid ${C.yellow}15` }}>
          <T color={C.yellow} bold center>Why is this efficient?</T>
          <T color="#ffe082" size={15} style={{ marginTop: 4 }}>The encoder runs <strong>once</strong>. K and V are computed <strong>once</strong>. Only Q changes per decoder layer. For a 1000-token input, this saves computing 1000-token K and V six times - a 6x reduction in encoder-side computation.</T>
        </div>
        <div style={{ flex: "1 1 200px", padding: "10px 12px", borderRadius: 8, background: `${C.red}06`, border: `1px solid ${C.red}15` }}>
          <T color="#ef9a9a" bold center>6 weight matrices per layer</T>
          <T color="#ef9a9a" size={15} style={{ marginTop: 4 }}>Each decoder layer has <strong>6</strong> separate learned matrices: W<sub>Q</sub>, W<sub>K</sub>, W<sub>V</sub> for masked self-attention (Q, K, V all from French) + W<sub>Q</sub>, W<sub>K</sub>, W<sub>V</sub> for cross-attention (Q from French, K and V from English). All learned independently.</T>
        </div>
      </div>
    </Box></Reveal>
    ...
    <Reveal when={sub >= 7}><Box color={C.green} style={{ width: "100%" }}>
      <T color="#80e8a5" bold center size={20}>Where does cross-attention live?</T>

      <div style={{ marginTop: 12, display: "flex", flexDirection: "column", gap: 10 }}>
        <div style={{ padding: "12px", borderRadius: 10, background: `${C.orange}06`, border: `1px solid ${C.orange}15`, display: "flex", flexDirection: "column", alignItems: "center" }}>
          <div style={{ padding: "3px 10px", borderRadius: 5, background: `${C.orange}20`, border: `1px solid ${C.orange}40` }}>
            <T color={C.orange} bold size={14} center>decoder-only</T>
          </div>
          <T color={C.dim} size={14} center style={{ marginTop: 4 }}>GPT, Claude, LLaMA</T>
          <T color="#ffb74d" bold center size={18} style={{ marginTop: 6 }}>No cross-attention at all</T>
          <T color={C.dim} size={15} center style={{ marginTop: 4 }}>There's no encoder, so nothing to cross-attend to. The decoder handles everything with just self-attention. Your prompt and the response are one continuous sequence.</T>
        </div>

        <div style={{ padding: "12px", borderRadius: 10, background: `${C.purple}06`, border: `1px solid ${C.purple}15`, display: "flex", flexDirection: "column", alignItems: "center" }}>
          <div style={{ padding: "3px 10px", borderRadius: 5, background: `${C.purple}20`, border: `1px solid ${C.purple}40` }}>
            <T color={C.purple} bold size={14} center>encoder-decoder</T>
          </div>
          <T color={C.dim} size={14} center style={{ marginTop: 4 }}>Original Transformer, T5, BART</T>
          <T color="#b8a9ff" bold center size={18} style={{ marginTop: 6 }}>Cross-attention in every decoder layer</T>
          <T color={C.dim} size={15} center style={{ marginTop: 4 }}>Each decoder layer has three sub-layers in order:</T>
          <div style={{ marginTop: 8, display: "flex", flexDirection: "column", gap: 4, width: "100%", maxWidth: 340 }}>
            {[
              ...
              <div key={n} style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 10px", borderRadius: 6, background: `${color}08`, border: `1px solid ${color}15` }}>
                <span style={{ color, fontWeight: 800, fontSize: 18, minWidth: 20 }}>{n}.</span>
                <div>
                  <T color={color} bold size={15}>{label}</T>
                  <T color={C.dim} size={13}>{desc}</T>
                </div>
              </div>
            ))}
          </div>
          <T color={C.dim} size={14} center style={{ marginTop: 6 }}>This is exactly what the architecture diagram in chapter 5.1 shows - the green arrow "K, V from encoder" connects to this cross-attention layer.</T>
        </div>
      </div>

      <div style={{ marginTop: 12, padding: "10px 12px", borderRadius: 8, background: `${C.green}06`, border: `1px solid ${C.green}15` }}>
        <T color="#80e8a5" bold center>Cross-attention is elegant but optional</T>
        <T color="#80e8a5" size={16} style={{ marginTop: 4 }}>Modern LLMs prove you can get world-class results without it. But for tasks with clear input/output separation (translation, summarization), the encoder-decoder architecture with cross-attention remains powerful.</T>
      </div>
    </Box></Reveal>
...
export const GroupedQueryAttention = (ctx) => { const { sub, subBtnRipple, setSubBtnRipple, registerSubBtn, navigate } = ctx; return (
  <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 16 }}>
    {sub >= 0 && (
      <Box color={C.blue} style={{ width: "100%" }}>
        <T color={C.blue} bold center size={20}>Recall: Multi-Head Attention</T>
        <T color={C.blue} style={{ marginTop: 6 }}>In chapters 7.9-7.12, we learned standard MHA. With 8 heads, the model maintains 8 independent sets of Q, K, V weight matrices:</T>
        <div style={{ marginTop: 12, display: "flex", flexDirection: "column", gap: 6 }}>
          <div style={{ display: "grid", gridTemplateColumns: "80px 1fr", gap: 8, padding: "8px 12px", borderRadius: 8, background: `${C.blue}06`, border: `1px solid ${C.blue}12` }}>
            <T color={C.blue} bold size={16}>Queries</T>
            <div style={{ display: "flex", gap: 3, flexWrap: "wrap" }}>
              {[1,2,3,4,5,6,7,8].map(i => (
                <div key={i} style={{ padding: "3px 8px", borderRadius: 4, background: `${C.blue}15`, border: `1px solid ${C.blue}25` }}>
                  <T color={C.blue} size={13} bold>W_Q{i}</T>
                </div>
              ))}
            </div>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "80px 1fr", gap: 8, padding: "8px 12px", borderRadius: 8, background: `${C.orange}06`, border: `1px solid ${C.orange}12` }}>
            <T color={C.orange} bold size={16}>Keys</T>
            <div style={{ display: "flex", gap: 3, flexWrap: "wrap" }}>
              {[1,2,3,4,5,6,7,8].map(i => (
                <div key={i} style={{ padding: "3px 8px", borderRadius: 4, background: `${C.orange}15`, border: `1px solid ${C.orange}25` }}>
                  <T color={C.orange} size={13} bold>W_K{i}</T>
                </div>
              ))}
            </div>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "80px 1fr", gap: 8, padding: "8px 12px", borderRadius: 8, background: `${C.green}06`, border: `1px solid ${C.green}12` }}>
            <T color={C.green} bold size={16}>Values</T>
            <div style={{ display: "flex", gap: 3, flexWrap: "wrap" }}>
              {[1,2,3,4,5,6,7,8].map(i => (
                <div key={i} style={{ padding: "3px 8px", borderRadius: 4, background: `${C.green}15`, border: `1px solid ${C.green}25` }}>
                  <T color={C.green} size={13} bold>W_V{i}</T>
                </div>
              ))}
            </div>
          </div>
        </div>
        <T color={C.blue} style={{ marginTop: 8 }}>That is 8 W_Q + 8 W_K + 8 W_V = <strong>24 weight matrices</strong> per attention layer. Each head has its own complete set. This works great for quality, but there is a hidden cost during inference.</T>
      </Box>
    )}

    <Reveal when={sub >= 1}><Box color={C.red} style={{ width: "100%" }}>
      <T color="#ef9a9a" bold center size={20}>The inference problem: KV cache memory</T>
      <T color="#ef9a9a" style={{ marginTop: 6 }}>During generation, we cache the K and V vectors for all previous tokens. This avoids recomputing them at every step. But this <strong>KV cache</strong> grows fast.</T>
      <div style={{ marginTop: 12, padding: "14px", borderRadius: 8, background: "rgba(0,0,0,0.3)" }}>
        <T color={C.bright} bold center size={15}>KV cache memory per layer</T>
      ...
      <div style={{ marginTop: 12, display: "flex", flexDirection: "column", gap: 4 }}>
        <T color="#ef9a9a" bold size={16}>Concrete example: LLaMA 2 70B</T>
        {[
          ...
          <div key={param} style={{ display: "grid", gridTemplateColumns: "90px 50px 1fr", gap: 8, padding: "4px 10px", borderRadius: 4, background: `${C.red}06` }}>
            <code style={{ color: C.red, fontSize: 14, fontWeight: 600 }}>{param}</code>
            <T color={C.mid} bold size={14}>{val}</T>
            <T color={C.dim} size={13}>{note}</T>
          </div>
        ))}
      </div>
      <div style={{ marginTop: 10, padding: "10px 14px", borderRadius: 8, background: `${C.yellow}06`, border: `1px solid ${C.yellow}12` }}>
        <T color={C.yellow} bold center size={16}>Total KV cache per batch item</T>
        <T color={C.dim} size={15} style={{ marginTop: 4, fontFamily: "monospace" }}>2 x 64 x 4096 x 128 x 2 bytes = 134,217,728 bytes per layer</T>
        <T color={C.dim} size={15} style={{ fontFamily: "monospace" }}>134,217,728 x 80 layers = 10,737,418,240 bytes</T>
        <T color={C.yellow} bold center size={18} style={{ marginTop: 6 }}>~10 GB per batch item!</T>
        <T color={C.dim} size={14} style={{ marginTop: 4 }}>Serving 8 users simultaneously = 80 GB just for KV caches. That is an entire A100 GPU of memory - before the model weights themselves.</T>
      </div>
    </Box></Reveal>

    <Reveal when={sub >= 2}><Box color={C.green} style={{ width: "100%" }}>
      <T color="#80e8a5" bold center size={20}>GQA solution: share K and V across query groups</T>
      <T color="#80e8a5" style={{ marginTop: 6 }}>Instead of giving every head its own K and V, we <strong>share</strong> K and V across groups of query heads. Each head still has its own unique Q - queries stay personal.</T>
      <div style={{ marginTop: 12, display: "flex", flexDirection: "column", gap: 10 }}>
        <div style={{ padding: "10px 14px", borderRadius: 8, background: `${C.purple}06`, border: `1px solid ${C.purple}12` }}>
          <T color={C.purple} bold center size={16}>Example: 8 query heads, 2 KV groups</T>
          <div style={{ marginTop: 10, display: "flex", flexDirection: "column", gap: 6 }}>
            <div style={{ padding: "8px 12px", borderRadius: 6, background: `${C.cyan}06`, border: `1px solid ${C.cyan}12` }}>
              <T color={C.cyan} bold size={15}>Group 1: Heads 1-4 share K_1, V_1</T>
              <div style={{ marginTop: 4, display: "flex", gap: 4, flexWrap: "wrap" }}>
                {["Q_1", "Q_2", "Q_3", "Q_4"].map(q => (
                  <div key={q} style={{ padding: "3px 8px", borderRadius: 4, background: `${C.blue}15`, border: `1px solid ${C.blue}25` }}>
                    <T color={C.blue} size={13} bold>{q}</T>
                  </div>
                ))}
                <T color={C.dim} size={14} style={{ alignSelf: "center" }}>all use</T>
                <div style={{ padding: "3px 8px", borderRadius: 4, background: `${C.orange}15`, border: `1px solid ${C.orange}25` }}>
                  <T color={C.orange} size={13} bold>K_1</T>
                </div>
                <div style={{ padding: "3px 8px", borderRadius: 4, background: `${C.green}15`, border: `1px solid ${C.green}25` }}>
                  <T color={C.green} size={13} bold>V_1</T>
                </div>
              </div>
            </div>
            <div style={{ padding: "8px 12px", borderRadius: 6, background: `${C.yellow}06`, border: `1px solid ${C.yellow}12` }}>
              <T color={C.yellow} bold size={15}>Group 2: Heads 5-8 share K_2, V_2</T>
              <div style={{ marginTop: 4, display: "flex", gap: 4, flexWrap: "wrap" }}>
                ...
                <T color={C.dim} size={14} style={{ alignSelf: "center" }}>all use</T>
                <div style={{ padding: "3px 8px", borderRadius: 4, background: `${C.orange}15`, border: `1px solid ${C.orange}25` }}>
                  <T color={C.orange} size={13} bold>K_2</T>
                </div>
                <div style={{ padding: "3px 8px", borderRadius: 4, background: `${C.green}15`, border: `1px solid ${C.green}25` }}>
                  <T color={C.green} size={13} bold>V_2</T>
                </div>
              </div>
            </div>
          </div>
        </div>
        <T color="#80e8a5" style={{ marginTop: 4 }}>Weight matrices needed: 8 W_Q + 2 W_K + 2 W_V = <strong>12 projections</strong> (down from 24). The K and V computations (and their caches) shrink by 4x.</T>
      </div>
    </Box></Reveal>

    <Reveal when={sub >= 3}><Box color={C.yellow} style={{ width: "100%" }}>
      <T color="#ffe082" bold center size={20}>Visual comparison: MHA vs GQA vs MQA</T>
      <div style={{ marginTop: 12, display: "flex", flexDirection: "column", gap: 10 }}>
        {[
          {
            name: "MHA (Multi-Head Attention)",
            qHeads: 8, kHeads: 8, vHeads: 8,
            projections: 24,
          ...
          },
          {
            ...
            qHeads: 8, kHeads: 1, vHeads: 1,
            projections: 10,
            kvCacheNote: "1 K + 1 V cached per layer (8x less)",
            color: C.blue,
          },
        ].map(({ name, qHeads, kHeads, vHeads, projections, kvCacheNote, color }) => (
          <div key={name} style={{ padding: "12px 14px", borderRadius: 8, background: `${color}06`, border: `1px solid ${color}12` }}>
            <T color={color} bold center size={17}>{name}</T>
            <div style={{ marginTop: 8, display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 6 }}>
              <div style={{ textAlign: "center" }}>
                <T color={C.blue} bold size={20} center>{qHeads}</T>
                <T color={C.dim} size={12} center>Q heads</T>
              </div>
              <div style={{ textAlign: "center" }}>
                <T color={C.orange} bold size={20} center>{kHeads}</T>
                <T color={C.dim} size={12} center>K heads</T>
              </div>
              <div style={{ textAlign: "center" }}>
                <T color={C.green} bold size={20} center>{vHeads}</T>
                <T color={C.dim} size={12} center>V heads</T>
              </div>
            </div>
            <T color={C.dim} size={14} center style={{ marginTop: 6 }}>{projections} projections total</T>
            <T color={C.mid} size={14} center style={{ marginTop: 2 }}>{kvCacheNote}</T>
          </div>
        ))}
      </div>
      <div style={{ marginTop: 12, padding: "10px 14px", borderRadius: 8, background: "rgba(0,0,0,0.3)" }}>
        <T color={C.bright} bold center size={15}>KV cache savings (LLaMA 2 70B, 4096 context)</T>
        <div style={{ marginTop: 8, display: "flex", flexDirection: "column", gap: 4 }}>
          {[
            ...
            <div key={type} style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <T color={color} size={13} style={{ minWidth: 150 }}>{type}</T>
              ...
              <T color={color} bold size={13} style={{ minWidth: 70, textAlign: "right" }}>{mem}</T>
            </div>
          ))}
        </div>
      </div>
    ...
    <Reveal when={sub >= 4}><Box color={C.purple} style={{ width: "100%" }}>
      <T color="#b8a9ff" bold center size={20}>Why does sharing K and V work?</T>
      <T color="#b8a9ff" style={{ marginTop: 6 }}>The query is the "question" each head asks. Each head needs its own unique question to capture different patterns (syntax, semantics, coreference, etc.).</T>
      <T color="#b8a9ff" style={{ marginTop: 6 }}>But keys and values are the "answers" - and it turns out adjacent heads often attend to <strong>similar positions</strong> anyway. The K and V vectors across heads are highly correlated. Sharing them loses very little information.</T>
      <div style={{ marginTop: 12, display: "flex", flexDirection: "column", gap: 8 }}>
        <div style={{ padding: "10px 14px", borderRadius: 8, background: `${C.green}06`, border: `1px solid ${C.green}12` }}>
          <T color={C.green} bold center size={16}>Research result</T>
          <T color={C.dim} size={15} style={{ marginTop: 4 }}>GQA matches standard MHA quality within 1% on most benchmarks while using 4x less KV cache memory. The quality-to-efficiency tradeoff is exceptionally favorable.</T>
        </div>
        <div style={{ padding: "10px 14px", borderRadius: 8, background: `${C.orange}06`, border: `1px solid ${C.orange}12` }}>
          <T color={C.orange} bold center size={16}>Models using GQA</T>
          <div style={{ marginTop: 6, display: "flex", gap: 6, flexWrap: "wrap", justifyContent: "center" }}>
            {[
              ...
              <div key={model} style={{ padding: "4px 10px", borderRadius: 5, background: `${C.orange}12`, border: `1px solid ${C.orange}25` }}>
                <T color={C.orange} bold size={14}>{model}</T>
                <T color={C.dim} size={11}>{detail}</T>
              </div>
            ))}
          </div>
        </div>
        <div style={{ padding: "10px 14px", borderRadius: 8, background: `${C.cyan}06`, border: `1px solid ${C.cyan}12` }}>
          <T color={C.cyan} bold center size={16}>Why not always use MQA (1 shared KV)?</T>
          <T color={C.dim} size={15} style={{ marginTop: 4 }}>MQA saves the most memory but does lose quality - about 2-5% on harder tasks. GQA is the sweet spot: most of the memory savings with almost no quality loss. That is why most modern large models use GQA with 8 groups, not full MQA.</T>
        </div>
      </div>
    </Box></Reveal>
...
export const KVCache = (ctx) => { const { sub, subBtnRipple, setSubBtnRipple, registerSubBtn, navigate } = ctx; return (
  <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 16 }}>

    {/* Sub 0: The inference cost problem */}
    {sub >= 0 && (
      <Box color={C.red} style={{ width: "100%" }}>
        <T color="#ef9a9a" bold center size={20}>The Inference Cost Problem</T>
        <T color="#ef9a9a" size={16} style={{ marginTop: 6 }}>During autoregressive generation (chapter 2.6), the model produces one token at a time. To generate token N, it must attend to ALL previous tokens 1 through N-1. Watch the cost grow:</T>

        <div style={{ marginTop: 14, padding: 14, borderRadius: 10, background: `${C.red}08`, border: `1px solid ${C.red}20` }}>
          <T color="#ef9a9a" bold center size={16}>Step-by-step generation cost (without cache):</T>
          <div style={{ marginTop: 10, display: "flex", flexDirection: "column", gap: 4 }}>
            {[
              { tok: 1, attn: "[1]", ops: 1, bar: 1 },
              { tok: 2, attn: "[1, 2]", ops: 4, bar: 4 },
              { tok: 3, attn: "[1, 2, 3]", ops: 9, bar: 9 },
              { tok: 4, attn: "[1, 2, 3, 4]", ops: 16, bar: 16 },
              { tok: 5, attn: "[1, 2, 3, 4, 5]", ops: 25, bar: 25 },
            ].map(({ tok, attn, ops, bar }) => (
              <div key={tok} style={{ display: "flex", alignItems: "center", gap: 8, padding: "4px 8px", borderRadius: 4, background: `${C.red}06` }}>
                <Tag color={C.red}>Token {tok}</Tag>
                <T color={C.dim} size={13}>attend to {attn}</T>
                <div style={{ flex: 1, height: 10, borderRadius: 3, background: "rgba(255,255,255,0.04)", overflow: "hidden" }}>
                  <div style={{ width: `${bar * 4}%`, height: "100%", borderRadius: 3, background: C.red }} />
                </div>
                <code style={{ color: C.red, fontSize: 12, minWidth: 40, textAlign: "right" }}>{ops} ops</code>
              </div>
            ))}
            <div style={{ padding: "4px 8px", borderRadius: 4, background: `${C.orange}06` }}>
              <T color={C.orange} size={13}>Token N: full self-attention over N tokens = N<sup>2</sup> operations per step</T>
            </div>
          </div>
        </div>

        <div style={{ marginTop: 12, padding: 10, borderRadius: 8, background: `${C.yellow}08`, border: `1px solid ${C.yellow}20` }}>
          <T color="#fff176" bold center size={15}>Total cost to generate N tokens (naive approach):</T>
          <T color="#fff176" center size={16} style={{ marginTop: 4 }}>1<sup>2</sup> + 2<sup>2</sup> + 3<sup>2</sup> + ... + N<sup>2</sup> = <strong>O(N<sup>3</sup>)</strong></T>
          <T color={C.dim} center size={14} style={{ marginTop: 4 }}>For N = 1000 tokens: roughly <strong style={{ color: C.red }}>333 million</strong> attention operations!</T>
        </div>
      </Box>
    )}
    {sub === 0 && <SubBtn key={sub} onClick={() => { setSubBtnRipple(Date.now()); navigate("forward"); }} rippleKey={subBtnRipple} registerSubBtn={registerSubBtn} />}

    {/* Sub 1: The key insight - redundant computation */}
    <Reveal when={sub >= 1}><Box color={C.yellow} style={{ width: "100%" }}>
      <T color="#fff176" bold center size={20}>The Key Insight: Redundant Computation</T>
      <T color="#fff176" size={16} style={{ marginTop: 6 }}>When generating token 5, we recompute Q, K, V for ALL tokens. But look what stays the same:</T>

      <div style={{ marginTop: 14, display: "flex", flexDirection: "column", gap: 8 }}>
        <div style={{ padding: 12, borderRadius: 8, background: `${C.orange}06`, border: `1px solid ${C.orange}15` }}>
          <T color={C.orange} bold center size={15}>Generating token 4 (previous step)</T>
          <div style={{ marginTop: 6, display: "flex", flexWrap: "wrap", gap: 4, justifyContent: "center" }}>
            {["K1", "K2", "K3"].map((k) => <Tag key={k} color={C.blue}>{k}</Tag>)}
            {["V1", "V2", "V3"].map((v) => <Tag key={v} color={C.green}>{v}</Tag>)}
          </div>
        </div>
        <div style={{ textAlign: "center", color: C.dim, fontSize: 14 }}>↓ next step ↓</div>
        <div style={{ padding: 12, borderRadius: 8, background: `${C.yellow}06`, border: `1px solid ${C.yellow}15` }}>
          <T color={C.yellow} bold center size={15}>Generating token 5 (current step)</T>
          <div style={{ marginTop: 6, display: "flex", flexWrap: "wrap", gap: 4, justifyContent: "center" }}>
            {["K1", "K2", "K3"].map((k) => <Tag key={k} color={C.blue}>{k}</Tag>)}
            <Tag color={C.red}>K4</Tag>
            {["V1", "V2", "V3"].map((v) => <Tag key={v} color={C.green}>{v}</Tag>)}
            <Tag color={C.red}>V4</Tag>
          </div>
          <T color={C.dim} center size={13} style={{ marginTop: 6 }}>K1, K2, K3 and V1, V2, V3 are <strong style={{ color: C.yellow }}>exactly the same</strong> as last step!</T>
        </div>
      </div>

      <div style={{ marginTop: 12, padding: 12, borderRadius: 8, background: `${C.red}08`, border: `1px solid ${C.red}20` }}>
        <T color="#ef9a9a" bold center size={15}>The waste</T>
        <T color="#ef9a9a" size={15} style={{ marginTop: 4 }}>K and V for tokens 1, 2, 3 haven't changed - they're computed from the same embeddings with the same W_K and W_V matrices. Yet we're recomputing them from scratch at every single generation step. For 1000 tokens, K1 gets recomputed 999 times for nothing.</T>
      </div>
    </Box></Reveal>
    {sub === 1 && <SubBtn key={sub} onClick={() => { setSubBtnRipple(Date.now()); navigate("forward"); }} rippleKey={subBtnRipple} registerSubBtn={registerSubBtn} />}

    {/* Sub 2: The KV Cache solution */}
    <Reveal when={sub >= 2}><Box color={C.green} style={{ width: "100%" }}>
      <T color="#a5d6a7" bold center size={20}>The KV Cache Solution</T>
      <T color="#a5d6a7" size={16} style={{ marginTop: 6 }}>Store K and V from all previous tokens in a cache. At each step:</T>

      <div style={{ marginTop: 14, display: "flex", flexDirection: "column", gap: 6 }}>
        {[
          { step: "1", desc: "Compute Q, K, V for ONLY the new token (1 token, not N)", color: C.cyan },
          { step: "2", desc: "Append the new K and V to the cache", color: C.blue },
          { step: "3", desc: "Compute attention: new Q against ALL cached K and V", color: C.green },
        ].map(({ step, desc, color }) => (
          <div key={step} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 12px", borderRadius: 8, background: `${color}06`, border: `1px solid ${color}15` }}>
            <Tag color={color}>Step {step}</Tag>
            <T color={C.dim} size={15}>{desc}</T>
          </div>
        ))}
      </div>

      <div style={{ marginTop: 14, padding: 14, borderRadius: 10, background: `${C.green}08`, border: `1px solid ${C.green}25` }}>
        <T color="#a5d6a7" bold center size={16}>Cost comparison:</T>
        <div style={{ marginTop: 10, display: "flex", gap: 12, flexWrap: "wrap" }}>
          <div style={{ flex: "1 1 180px", padding: 10, borderRadius: 8, background: `${C.red}06`, border: `1px solid ${C.red}15` }}>
            <T color={C.red} bold center size={14}>Without cache</T>
            <T color={C.dim} size={13} style={{ marginTop: 4 }}>Each step: recompute ALL N tokens' K, V</T>
            <T color={C.dim} size={13}>Then full N x N attention</T>
            <T color={C.red} bold center size={14} style={{ marginTop: 6 }}>Per step: O(N<sup>2</sup>)</T>
            <T color={C.red} bold center size={14}>Total: O(N<sup>3</sup>)</T>
          </div>
          <div style={{ flex: "1 1 180px", padding: 10, borderRadius: 8, background: `${C.green}06`, border: `1px solid ${C.green}15` }}>
            <T color={C.green} bold center size={14}>With KV cache</T>
            <T color={C.dim} size={13} style={{ marginTop: 4 }}>Each step: compute ONLY 1 token's K, V</T>
            <T color={C.dim} size={13}>Then 1 x N attention (one Q vs N keys)</T>
            <T color={C.green} bold center size={14} style={{ marginTop: 6 }}>Per step: O(N)</T>
            <T color={C.green} bold center size={14}>Total: O(N<sup>2</sup>)</T>
          </div>
        </div>

        <div style={{ marginTop: 10, padding: 8, borderRadius: 6, background: `${C.yellow}08`, border: `1px solid ${C.yellow}20` }}>
          <T color="#fff176" bold center size={15}>N = 1000 tokens</T>
          <T color={C.dim} center size={14} style={{ marginTop: 4 }}>Without cache: ~333 million ops. With cache: ~500 thousand ops. <strong style={{ color: C.green }}>~660x faster.</strong></T>
        </div>
      </div>
    </Box></Reveal>
    {sub === 2 && <SubBtn key={sub} onClick={() => { setSubBtnRipple(Date.now()); navigate("forward"); }} rippleKey={subBtnRipple} registerSubBtn={registerSubBtn} />}

    {/* Sub 3: Visual - the cache growing step by step */}
    <Reveal when={sub >= 3}><Box color={C.purple} style={{ width: "100%" }}>
      <T color="#b8a9ff" bold center size={20}>The Cache Growing Step by Step</T>
      <T color="#b8a9ff" size={16} style={{ marginTop: 6 }}>Watch the KV cache grow as the model generates "I love cats":</T>
        ...
        {[
          {
            step: 1, word: '"I"',
            cache: { k: ["K_I"], v: ["V_I"] },
            compute: "Compute Q_I, K_I, V_I from scratch",
            attn: "Q_I attends to [K_I]",
            color: C.red,
          },
          {
            step: 2, word: '"love"',
            cache: { k: ["K_I", "K_love"], v: ["V_I", "V_love"] },
            compute: "Compute Q_love, K_love, V_love only",
            attn: "Q_love attends to [K_I, K_love]",
            color: C.purple,
          },
          {
            step: 3, word: '"cats"',
            cache: { k: ["K_I", "K_love", "K_cats"], v: ["V_I", "V_love", "V_cats"] },
            compute: "Compute Q_cats, K_cats, V_cats only",
            attn: "Q_cats attends to [K_I, K_love, K_cats]",
            color: C.cyan,
          },
        ].map(({ step, word, cache, compute, attn, color }) => (
          <div key={step} style={{ padding: 12, borderRadius: 10, background: `${color}06`, border: `1px solid ${color}20` }}>
            <T color={color} bold size={15}>Step {step}: Generate {word}</T>
            <div style={{ marginTop: 6, display: "flex", flexDirection: "column", gap: 4 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "4px 8px", borderRadius: 4, background: `${C.orange}06` }}>
                <T color={C.orange} size={12} bold>Compute:</T>
                <T color={C.dim} size={12}>{compute}</T>
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "4px 8px", borderRadius: 4, background: `${C.blue}06` }}>
                <T color={C.blue} size={12} bold>K cache:</T>
                <div style={{ display: "flex", gap: 3 }}>{cache.k.map((k) => <Tag key={k} color={C.blue}>{k}</Tag>)}</div>
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "4px 8px", borderRadius: 4, background: `${C.green}06` }}>
                <T color={C.green} size={12} bold>V cache:</T>
                <div style={{ display: "flex", gap: 3 }}>{cache.v.map((v) => <Tag key={v} color={C.green}>{v}</Tag>)}</div>
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "4px 8px", borderRadius: 4, background: `${C.pink}06` }}>
                <T color={C.pink} size={12} bold>Attention:</T>
                <T color={C.dim} size={12}>{attn}</T>
              </div>
            </div>
          </div>
        ))}
      ...
      <div style={{ marginTop: 10, padding: 10, borderRadius: 8, background: `${C.purple}08`, border: `1px solid ${C.purple}20` }}>
        <T color="#b8a9ff" size={14} center>At each step, only the <strong>new token's</strong> Q, K, V are computed. Old K and V entries are never recomputed - they're read directly from the cache. The cache grows by one K and one V entry per step.</T>
      </div>
    </Box></Reveal>
    {sub === 3 && <SubBtn key={sub} onClick={() => { setSubBtnRipple(Date.now()); navigate("forward"); }} rippleKey={subBtnRipple} registerSubBtn={registerSubBtn} />}

    {/* Sub 4: The memory cost */}
    <Reveal when={sub >= 4}><Box color={C.orange} style={{ width: "100%" }}>
      <T color="#ffcc80" bold center size={20}>The Memory Cost</T>
      <T color="#ffcc80" size={16} style={{ marginTop: 6 }}>KV cache is not free. Each layer needs to store K and V for every token generated so far.</T>

      <div style={{ marginTop: 14, padding: 14, borderRadius: 10, background: `${C.orange}08`, border: `1px solid ${C.orange}25` }}>
        <T color="#ffcc80" bold center size={16}>Cache size per layer:</T>
        <div style={{ marginTop: 8, display: "flex", alignItems: "center", justifyContent: "center", gap: 6, flexWrap: "wrap" }}>
          <Tag color={C.blue}>2</Tag>
          <T color={C.dim} size={14}>(K + V)</T>
          <T color={C.dim} size={14} bold>x</T>
          <Tag color={C.cyan}>seq_length</Tag>
          <T color={C.dim} size={14} bold>x</T>
          <Tag color={C.purple}>d_model</Tag>
          <T color={C.dim} size={14} bold>x</T>
          <Tag color={C.orange}>bytes</Tag>
        </div>
      </div>

      <div style={{ marginTop: 14, padding: 14, borderRadius: 10, background: "rgba(0,0,0,0.25)", border: `1px solid ${C.dim}20` }}>
        <T color={C.dim} bold center size={15}>Example: LLaMA 70B</T>
        <div style={{ marginTop: 8, display: "flex", flexDirection: "column", gap: 4 }}>
          {[
            ...
            <div key={param} style={{ display: "flex", alignItems: "center", gap: 8, padding: "4px 10px", borderRadius: 4, background: `${color}06` }}>
              <T color={color} bold size={13} style={{ minWidth: 100 }}>{param}</T>
              <T color={C.dim} size={13}>{val}</T>
            </div>
          ))}
        </div>

        <div style={{ marginTop: 10, padding: 8, borderRadius: 6, background: `${C.red}08`, border: `1px solid ${C.red}20` }}>
          <T color={C.dim} center size={13}>Cache = 2 x 4,096 x 8,192 x 80 x 2 bytes</T>
          <T color={C.red} bold center size={16} style={{ marginTop: 4 }}>= 10.7 GB per sequence</T>
        </div>
      </div>

      <div style={{ marginTop: 12, display: "flex", flexDirection: "column", gap: 4 }}>
        <T color="#ffcc80" bold size={15}>This is why:</T>
        {[
          { fact: "Long context is expensive", detail: "Doubling the sequence length doubles the KV cache. 128K context = enormous memory.", color: C.red },
          { fact: "Batch size is limited", detail: "Each concurrent user needs their own KV cache. 10 users = 107 GB just for caches.", color: C.orange },
          { fact: "GQA helps", detail: "Grouped-Query Attention shares K, V across multiple Q heads, shrinking the cache by 4-8x.", color: C.green },
        ].map(({ fact, detail, color }) => (
          <div key={fact} style={{ display: "flex", gap: 6, alignItems: "flex-start", padding: "6px 10px", borderRadius: 6, background: `${color}06`, border: `1px solid ${color}12` }}>
            <span style={{ color, fontSize: 10, marginTop: 4 }}>&#9679;</span>
            <T color={C.dim} size={13}><strong style={{ color }}>{fact}:</strong> {detail}</T>
          </div>
        ))}
      </div>
    </Box></Reveal>
    ...
    {/* Sub 5: The tradeoff */}
    <Reveal when={sub >= 5}><Box color={C.cyan} style={{ width: "100%" }}>
      <T color="#80deea" bold center size={20}>The Fundamental Tradeoff</T>
      <T color="#80deea" size={16} style={{ marginTop: 6 }}>KV cache trades <strong>memory</strong> for <strong>speed</strong>. This is the central tension of LLM inference:</T>

      <div style={{ marginTop: 14, display: "flex", gap: 12, flexWrap: "wrap" }}>
        <div style={{ flex: "1 1 200px", padding: 14, borderRadius: 10, background: `${C.red}06`, border: `1px solid ${C.red}20` }}>
          <T color={C.red} bold center size={16}>Without KV Cache</T>
          <div style={{ marginTop: 8, display: "flex", flexDirection: "column", gap: 4 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
              <T color={C.red} size={13} bold>Speed:</T>
              <T color={C.dim} size={13}>Slow (recompute everything)</T>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
              <T color={C.green} size={13} bold>Memory:</T>
              <T color={C.dim} size={13}>Low (no cache stored)</T>
            </div>
          </div>
          <T color={C.dim} center size={12} style={{ marginTop: 8 }}>O(N<sup>3</sup>) compute, O(1) extra memory</T>
        </div>
        <div style={{ flex: "1 1 200px", padding: 14, borderRadius: 10, background: `${C.green}06`, border: `1px solid ${C.green}20` }}>
          <T color={C.green} bold center size={16}>With KV Cache</T>
          <div style={{ marginTop: 8, display: "flex", flexDirection: "column", gap: 4 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
              <T color={C.green} size={13} bold>Speed:</T>
              <T color={C.dim} size={13}>Fast (only compute new token)</T>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
              <T color={C.red} size={13} bold>Memory:</T>
              <T color={C.dim} size={13}>High (GBs per sequence)</T>
            </div>
          </div>
          <T color={C.dim} center size={12} style={{ marginTop: 8 }}>O(N<sup>2</sup>) compute, O(N) extra memory</T>
        </div>
      </div>

      <div style={{ marginTop: 14, padding: 12, borderRadius: 10, background: `${C.cyan}08`, border: `1px solid ${C.cyan}25` }}>
        <T color="#80deea" bold center size={16}>The Bottleneck of LLM Inference</T>
        <T color="#80deea" size={15} style={{ marginTop: 6 }}>This memory-speed tradeoff is the fundamental bottleneck of LLM serving. Every technique you hear about - GQA (Grouped-Query Attention), quantized KV cache, sliding window attention, paged attention - exists to reduce the memory cost of the KV cache while keeping the speed benefit. When companies say "serving LLMs is expensive," the KV cache is a major reason why.</T>
      </div>

      <div style={{ marginTop: 10, padding: 10, borderRadius: 8, background: `${C.purple}08`, border: `1px solid ${C.purple}20` }}>
        <T color="#b8a9ff" size={14} center>Note: KV cache only applies to <strong>inference</strong> (generation). During training, all tokens are processed in parallel, so there's no sequential generation and no need for a cache.</T>
      </div>
    </Box></Reveal>
  </div>
); };
