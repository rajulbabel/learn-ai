...
// ============================================================================
export const SelfSupervised = (ctx) => {
  const { sub, subBtnRipple, setSubBtnRipple, registerSubBtn, navigate } = ctx;
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 16 }}>
      {sub >= 0 && (
...
const SORTED_PROBS = Object.entries(SCORES)
  .map(([w, s]) => [w, Math.exp(s) / EXP_SUM])
  .sort((a, b) => b[1] - a[1]);

export const NNInAction = (ctx) => {
  const { sub, subBtnRipple, setSubBtnRipple, registerSubBtn, navigate } = ctx;
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 16 }}>
    ...
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 16 }}>
      ...
      <Reveal when={sub >= 4}>
        <Box color={C.green} style={{ width: "100%" }}>
          ...
          <T color="#80e8a5" style={{ marginTop: 8 }}>
            Now we have a machine that can grade any response automatically. This is what PPO will use in Step 4 -
...
// ============================================================================
export const BatchTraining = (ctx) => {
  const { sub, subBtnRipple, setSubBtnRipple, registerSubBtn, navigate } = ctx;
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 16 }}>
      {sub >= 0 && (
        ...
        <Box color={C.cyan} style={{ width: "100%" }}>
          ...
          <div style={{ marginTop: 14, display: "flex", flexDirection: "column", gap: 10 }}>
            ...
            >
              ...
              <T color={C.dim} size={14} style={{ marginTop: 6 }}>
                Each word gets converted to a vector of 768 numbers (the embedding from chapter 2.1). So "the" becomes
                [-0.14, 0.62, -0.38, ...]
              </T>
            </div>
      ...
      <Reveal when={sub >= 7}>
        <Box color={C.blue} style={{ width: "100%" }}>
          ...
          <T color="#5eb3ff" style={{ marginTop: 6 }}>
            Remember the embedding layer from chapter 2.1? It converts tokens to vectors. The embedding matrix has shape
...
// ============================================================================
export const AutoregressiveGeneration = (ctx) => {
  const { sub, subBtnRipple, setSubBtnRipple, registerSubBtn, navigate } = ctx;
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 16 }}>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 16 }}>
      {sub >= 0 && (
        <Box color={C.purple} style={{ width: "100%" }}>
          <T color="#b8a9ff" bold center size={20}>
            Recall RLHF - 4 moving pieces
          </T>
          <T color="#b8a9ff" style={{ marginTop: 6 }}>
            In chapter 2.8 we learned the RLHF pipeline. It works, but it is <strong>complex</strong>. Let's trace the
            full pipeline and see where the pain points are.
          </T>
          <div style={{ marginTop: 12, display: "flex", flexDirection: "column", gap: 8 }}>
            {[
              ...
              { step: "2. Train reward model", desc: "Separate neural net learns to score responses", color: C.yellow },
              {
                ...
                desc: "Policy model maximizes reward while staying close to reference",
                color: C.red,
              },
              {
                step: "4. Repeat & stabilize",
                desc: "Balance reward hacking vs quality - tricky to tune",
                color: C.pink,
              },
            ].map(({ step, desc, color }) => (
              <div
          ...
          <T color="#b8a9ff" style={{ marginTop: 10 }}>
            That is <strong>4 models running simultaneously</strong>: the policy (the LLM being trained), a frozen
            reference copy, the reward model, and a value model for PPO. Unstable RL training, risk of reward hacking.
            Can we do better?
          </T>
        </Box>
      )}
      ...
      <Reveal when={sub >= 1}>
        <Box color={C.green} style={{ width: "100%" }}>
          <T color="#80e8a5" bold center size={20}>
            DPO insight: skip the reward model entirely
          </T>
          <T color="#80e8a5" style={{ marginTop: 6 }}>
            Key paper: <strong>"Direct Preference Optimization"</strong> (Rafailov et al., 2023).
          </T>
          <T color="#80e8a5" style={{ marginTop: 8 }}>
            The insight is elegant: given human preference pairs (response A is better than response B), we can derive
            the optimal policy <strong>directly</strong> from those pairs - no reward model needed.
          </T>
          <div
            style={{
              marginTop: 12,
          ...
          >
            ...
            <T color={C.dim} size={15} style={{ marginTop: 6 }}>
              The reward model and the optimal policy are mathematically linked. If you know the optimal policy, you can
              derive what the reward model WOULD have been. So instead of training a separate reward model and then
              doing RL, you can fold both steps into a single supervised loss.
            </T>
          </div>
          <div style={{ marginTop: 10, display: "flex", gap: 8, alignItems: "stretch" }}>
            ...
            >
              ...
              <T color={C.dim} size={14} style={{ marginTop: 4 }}>
                Preferences → Reward model → RL → Policy
              </T>
              <T color={C.dim} size={13} style={{ marginTop: 2 }}>
      ...
      <Reveal when={sub >= 2}>
        <Box color={C.yellow} style={{ width: "100%" }}>
          ...
          <T color="#ffe082" style={{ marginTop: 6 }}>
            For each preference pair where y_w is the preferred response and y_l is the rejected response, given prompt
            x:
          </T>
          ...
          <div style={{ marginTop: 12, display: "flex", flexDirection: "column", gap: 6 }}>
            {[
              ...
              {
                sym: "pi_ref(y|x)",
                desc: "Probability the REFERENCE model assigns (frozen copy from before DPO training)",
                color: C.blue,
              },
              {
                sym: "B (beta)",
                desc: "Controls how far we can deviate from reference - higher B = stay closer to reference",
                color: C.orange,
              },
              ...
              >
                <code style={{ color, fontWeight: 700, fontSize: 16, minWidth: 100, fontFamily: "monospace" }}>
                  {sym}
                </code>
                <T color={C.dim} size={15}>
                  {desc}
          ...
          >
            <T color={C.green} bold center size={16}>
              When does loss go down?
            </T>
            <T color={C.dim} size={15} style={{ marginTop: 6 }}>
              The inner term is: log(pi(y_w|x)/pi_ref(y_w|x)) - log(pi(y_l|x)/pi_ref(y_l|x))
            </T>
            <T color={C.dim} size={15} style={{ marginTop: 4 }}>
              This is positive when the model increases the probability of the preferred response AND decreases the
              probability of the rejected response (relative to reference). Sigmoid maps it to (0,1), then -log makes it
              a loss. Bigger gap between preferred and rejected = lower loss.
            </T>
          </div>
        </Box>
      </Reveal>
      ...
      <Reveal when={sub >= 3}>
        <Box color={C.cyan} style={{ width: "100%" }}>
          ...
          <div style={{ marginTop: 12, display: "flex", flexDirection: "column", gap: 4 }}>
            ...
            {[
              ...
              },
              {
                ...
                rlhf: "Real risk - model exploits reward model",
                dpo: "No reward model to hack",
                labelColor: C.mid,
              },
              {
                label: "Hyperparameters",
                rlhf: "Many (PPO clip, learning rate, KL penalty...)",
                dpo: "Few (mainly beta)",
                labelColor: C.mid,
              },
              {
                label: "Memory usage",
                rlhf: "4 models in GPU memory",
                dpo: "2 models in GPU memory",
                labelColor: C.mid,
              },
              ...
              >
                <T color={labelColor} bold size={14}>
                  {label}
                </T>
                <T color={C.dim} size={14}>
                  {rlhf}
          ...
          <T color="#80deea" style={{ marginTop: 10 }}>
            Same quality, a fraction of the complexity. This is why DPO took over so quickly.
          </T>
        </Box>
      </Reveal>
            ...
            <div style={{ marginTop: 8, display: "flex", flexDirection: "column", gap: 4 }}>
              {[
                ...
                <div key={name} style={{ display: "flex", gap: 8, alignItems: "center", padding: "4px 8px" }}>
                  <T color={C.yellow} bold size={16} style={{ minWidth: 50 }}>
                    {name}
                  </T>
                  <T color={C.dim} size={14}>
                    {desc}
...
// ============================================================================
export const TokenizerDeepDive = (ctx) => {
  const { sub, subBtnRipple, setSubBtnRipple, registerSubBtn, navigate } = ctx;
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 16 }}>
      {sub >= 0 && (
