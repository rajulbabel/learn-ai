...
export const Graph = ({ points, color, width = 300, height = 140, xLabel = "", yLabel = "", title = "", annotations = [] }) => {
...
export const BackpropRealNetwork = (ctx) => {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 16 }}>
      ...
      {sub >= 0 && (
        <Box color={C.red} style={{ width: "100%" }}>
          <T color={C.red} bold center size={20}>Our Network Got It Wrong - Now Fix It</T>
          <T color="#ef9a9a" size={16} style={{ marginTop: 12 }}>In chapter 1.7, we traced a forward pass. The network predicted a house price:</T>
          <div style={{ marginTop: 14, display: "flex", flexDirection: "column", gap: 10, alignItems: "center" }}>
            <div style={{ display: "flex", gap: 10, justifyContent: "center" }}>
              {[
                ...
                <div key={label} style={{ padding: "10px 16px", background: `${c}06`, borderRadius: 8, border: `1px solid ${c}12`, textAlign: "center", minWidth: 110 }}>
                  <T color={C.dim} size={12} center>{label}</T>
                  <T color={c} bold size={16} center style={{ marginTop: 4 }}>{val}</T>
                </div>
              ))}
            </div>
            <div style={{ display: "flex", gap: 10, justifyContent: "center" }}>
              <div style={{ padding: "10px 16px", background: `${C.yellow}06`, borderRadius: 8, border: `1px solid ${C.yellow}12`, textAlign: "center", minWidth: 110 }}>
                <T color={C.dim} size={12} center>Target</T>
                <T color={C.yellow} bold size={16} center style={{ marginTop: 4 }}>900</T>
              </div>
              <div style={{ padding: "10px 16px", background: `${C.red}08`, borderRadius: 8, border: `1px solid ${C.red}20`, textAlign: "center", minWidth: 150 }}>
                <T color={C.dim} size={12} center>Loss = (807 - 900)²</T>
                <T color={C.red} bold size={20} center style={{ marginTop: 4 }}>8,649</T>
              </div>
            </div>
          </div>
          <T color="#ef9a9a" size={16} style={{ marginTop: 12 }}>The network predicted 807 but the real price is 900. Loss = 8,649. How do we fix this? We need to figure out which weights are to blame and how to adjust each one.</T>
        </Box>
      )}
      ...
      <Reveal when={sub >= 1}>
        <Box color={C.purple} style={{ width: "100%" }}>
          <T color={C.purple} bold center size={20}>The Network - Now We Trace Backwards</T>
          <T color="#b8a9ff" size={16} style={{ marginTop: 10 }}>This is the same network from chapter 1.7. Every weight is labeled on its edge. The backward pass will compute a gradient for each one.</T>
            ...
            <svg viewBox="0 0 480 270" style={{ display: "block", width: "100%", maxWidth: 480 }}>
              ...
              {[
                ...
                return (
                  <g key={`h${i}`}>
                    <line x1={x1} y1={sy} x2={x2} y2={dy} stroke={`${c}30`} strokeWidth={1.2} />
                    <rect x={lx - 18} y={ly - 9} width={36} height={17} rx={3} fill="#08080dee" stroke={`${c}30`} strokeWidth={0.5} />
                    <text x={lx} y={ly + 4} fill={c} fontSize={9} textAnchor="middle" fontWeight={600}>{w}</text>
                  </g>
                );
              })}
              ...
              <text x={390} y={196} fill={C.yellow} fontSize={11} textAnchor="middle">target = 900</text>
              <text x={390} y={212} fill={C.red} fontSize={12} textAnchor="middle" fontWeight={700}>Loss = 8,649</text>

              {/* Backward arrow annotation */}
              <text x={240} y={258} fill={C.red} fontSize={11} textAnchor="middle" fontWeight={600}>{"← Gradients flow backwards through this network ←"}</text>
            </svg>
          </div>
          <T color="#b8a9ff" size={16} style={{ marginTop: 8 }}>The chain rule traces backward through every operation, multiplying derivatives at each step. Think of it like tracing blame: "Loss, who caused you? The prediction. Prediction, who caused you? The weights and hidden neurons..."</T>
        </Box>
      </Reveal>
      ...
      <Reveal when={sub >= 2}>
        <Box color={C.red} style={{ width: "100%" }}>
          <T color={C.red} bold center size={20}>Step 1: How Does Loss Change with y?</T>
          <T color="#ef9a9a" size={16} style={{ marginTop: 10 }}>Start at the very end. Loss = (y - target)². How much does loss change when y changes by a tiny amount?</T>
          <div style={{ marginTop: 12, padding: "14px", background: "rgba(0,0,0,0.4)", borderRadius: 12, border: `1px solid ${C.red}25` }}>
            <T color={C.dim} size={12} center style={{ textTransform: "uppercase", letterSpacing: 2, marginBottom: 8 }}>Derivative of Loss with respect to y</T>
            <div style={{ display: "flex", flexDirection: "column", gap: 6, alignItems: "center" }}>
              <T color={C.dim} size={16}>loss = (y - target)²</T>
              <T color={C.dim} size={16}>dLoss/dy = 2 x (y - target)</T>
              <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap", justifyContent: "center" }}>
                <T color={C.dim} size={16}>dLoss/dy = 2 x (</T>
                <span style={{ padding: "2px 8px", background: `${C.green}15`, borderRadius: 4, color: C.green, fontSize: 16, fontWeight: 600 }}>807</span>
                <T color={C.dim} size={16}>-</T>
                <span style={{ padding: "2px 8px", background: `${C.yellow}15`, borderRadius: 4, color: C.yellow, fontSize: 16, fontWeight: 600 }}>900</span>
                <T color={C.dim} size={16}>) = 2 x (-93) =</T>
                <span style={{ padding: "3px 10px", background: `${C.red}15`, borderRadius: 4, color: C.red, fontSize: 18, fontWeight: 700 }}>-186</span>
              </div>
            </div>
          </div>
          <T color="#ef9a9a" size={16} style={{ marginTop: 12 }}>This is negative (-186). It means: if we increased y by 1 unit, loss would decrease by 186. Makes sense - y = 807 is too low, so pushing it up reduces the error.</T>
        </Box>
      </Reveal>
      ...
      <Reveal when={sub >= 3}>
        <Box color={C.cyan} style={{ width: "100%" }}>
          <T color={C.cyan} bold center size={20}>Step 2: Output Layer Weight Gradients</T>
          <T color="#80deea" size={16} style={{ marginTop: 10 }}>Now trace one step further back. y = h₁ x w_o1 + h₂ x w_o2 + b_o. How does each output weight affect loss?</T>
          <div style={{ marginTop: 12, display: "flex", flexDirection: "column", gap: 10 }}>
            {[
              { name: "w_o1", formula: "dLoss/dy x dy/dw_o1 = dLoss/dy x h\u2081", calc: "-186 x 950", result: "-176,700", c: C.cyan },
              { name: "w_o2", formula: "dLoss/dy x dy/dw_o2 = dLoss/dy x h\u2082", calc: "-186 x 185", result: "-34,410", c: C.orange },
              { name: "b_o", formula: "dLoss/dy x dy/db_o = dLoss/dy x 1", calc: "-186 x 1", result: "-186", c: C.purple },
            ].map(({ name, formula, calc, result, c }) => (
              <div key={name} style={{ padding: "10px 12px", borderRadius: 8, background: `${c}06`, border: `1px solid ${c}12` }}>
                <T color={c} bold size={16}>dLoss/d{name}:</T>
                <T color={C.dim} size={14} style={{ marginTop: 4 }}>{formula}</T>
                <div style={{ marginTop: 4, display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
                  <T color={C.dim} size={15}>= {calc} =</T>
                  <span style={{ padding: "2px 10px", background: `${c}15`, borderRadius: 4, color: c, fontSize: 16, fontWeight: 700 }}>{result}</span>
                </div>
              </div>
            ))}
          </div>
          <T color="#80deea" size={16} style={{ marginTop: 10 }}>Why is the gradient for w_o1 five times larger than w_o2? Because h₁ = 950 is five times larger than h₂ = 185. A bigger input means the weight has more influence on the output.</T>
        </Box>
      </Reveal>
      ...
      <Reveal when={sub >= 4}>
        <Box color={C.yellow} style={{ width: "100%" }}>
          <T color={C.yellow} bold center size={20}>Step 3: How Does Gradient Reach the Hidden Layer?</T>
          <T color="#ffe082" size={16} style={{ marginTop: 10 }}>The output layer is done. But what about the hidden layer weights (w₁₁, w₂₁, etc.)? They are TWO steps away from loss. The gradient must flow backward through the output neuron first.</T>
          <div style={{ marginTop: 12, display: "flex", flexDirection: "column", gap: 10 }}>
            <div style={{ padding: "10px 12px", borderRadius: 8, background: `${C.cyan}06`, border: `1px solid ${C.cyan}12` }}>
              <T color={C.cyan} bold size={16}>How much does loss care about h₁?</T>
              <T color={C.dim} size={14} style={{ marginTop: 4 }}>y = h₁ x w_o1 + ..., so dy/dh₁ = w_o1 = 0.8</T>
              <div style={{ marginTop: 4, display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
                <T color={C.dim} size={15}>dLoss/dh₁ = dLoss/dy x w_o1 = -186 x 0.8 =</T>
                <span style={{ padding: "2px 10px", background: `${C.cyan}15`, borderRadius: 4, color: C.cyan, fontSize: 16, fontWeight: 700 }}>-148.8</span>
              </div>
            </div>
            <div style={{ padding: "10px 12px", borderRadius: 8, background: `${C.orange}06`, border: `1px solid ${C.orange}12` }}>
              <T color={C.orange} bold size={16}>How much does loss care about h₂?</T>
              <T color={C.dim} size={14} style={{ marginTop: 4 }}>y = ... + h₂ x w_o2 + ..., so dy/dh₂ = w_o2 = 0.2</T>
              <div style={{ marginTop: 4, display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
                <T color={C.dim} size={15}>dLoss/dh₂ = dLoss/dy x w_o2 = -186 x 0.2 =</T>
                <span style={{ padding: "2px 10px", background: `${C.orange}15`, borderRadius: 4, color: C.orange, fontSize: 16, fontWeight: 700 }}>-37.2</span>
              </div>
            </div>
          </div>
          <T color="#ffe082" size={16} style={{ marginTop: 10 }}>Notice: h₁ gets 4x more gradient than h₂ because w_o1 (0.8) is 4x bigger than w_o2 (0.2). The weight connecting h₁ to the output is like a pipe - a bigger weight means a wider pipe that carries more gradient backward.</T>
        </Box>
      </Reveal>
      ...
      <Reveal when={sub >= 5}>
        <Box color={C.green} style={{ width: "100%" }}>
          <T color={C.green} bold center size={20}>Step 4: Through ReLU - The Gate</T>
          <T color="#80e8a5" size={16} style={{ marginTop: 10 }}>Before reaching the hidden layer weights, the gradient must pass through ReLU. Remember: h₁ = ReLU(z₁) where z₁ is the pre-activation value.</T>
          <div style={{ marginTop: 12, display: "flex", gap: 10, flexWrap: "wrap", justifyContent: "center" }}>
            <div style={{ padding: "12px", background: `${C.green}06`, borderRadius: 8, border: `1px solid ${C.green}12`, flex: 1, minWidth: 180 }}>
              <T color={C.green} bold center size={16}>ReLU Derivative</T>
              <div style={{ marginTop: 8, display: "flex", flexDirection: "column", gap: 4 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  <span style={{ color: C.green, fontSize: 14, fontWeight: 600 }}>If z &gt; 0:</span>
                  <span style={{ padding: "2px 8px", background: `${C.green}15`, borderRadius: 4, color: C.green, fontSize: 16, fontWeight: 700 }}>derivative = 1</span>
                </div>
                <T color="#80e8a5" size={13}>Gradient passes through unchanged</T>
                <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 4 }}>
                  <span style={{ color: C.red, fontSize: 14, fontWeight: 600 }}>If z ≤ 0:</span>
                  <span style={{ padding: "2px 8px", background: `${C.red}15`, borderRadius: 4, color: C.red, fontSize: 16, fontWeight: 700 }}>derivative = 0</span>
                </div>
                <T color="#ef9a9a" size={13}>Gradient is completely blocked. Dead.</T>
              </div>
            </div>
          </div>
          <div style={{ marginTop: 12, display: "flex", flexDirection: "column", gap: 8 }}>
            <div style={{ padding: "10px 12px", borderRadius: 8, background: `${C.cyan}06`, border: `1px solid ${C.cyan}12` }}>
              <T color={C.cyan} bold size={15}>Neuron h₁: z₁ = 950 (positive) - ReLU derivative = 1</T>
              <div style={{ marginTop: 4, display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
                <T color={C.dim} size={15}>dLoss/dz₁ = dLoss/dh₁ x 1 = -148.8 x 1 =</T>
                <span style={{ padding: "2px 10px", background: `${C.cyan}15`, borderRadius: 4, color: C.cyan, fontSize: 16, fontWeight: 700 }}>-148.8</span>
              </div>
            </div>
            <div style={{ padding: "10px 12px", borderRadius: 8, background: `${C.orange}06`, border: `1px solid ${C.orange}12` }}>
              <T color={C.orange} bold size={15}>Neuron h₂: z₂ = 185 (positive) - ReLU derivative = 1</T>
              <div style={{ marginTop: 4, display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
                <T color={C.dim} size={15}>dLoss/dz₂ = dLoss/dh₂ x 1 = -37.2 x 1 =</T>
                <span style={{ padding: "2px 10px", background: `${C.orange}15`, borderRadius: 4, color: C.orange, fontSize: 16, fontWeight: 700 }}>-37.2</span>
              </div>
            </div>
          </div>
          <T color="#80e8a5" size={16} style={{ marginTop: 10 }}>Both neurons had positive pre-activation values, so ReLU let the full gradient through. But if z had been negative (like -50), ReLU would have output 0 during the forward pass, and the gradient would be completely blocked during backprop. That neuron would learn nothing from this example - a "dead neuron."</T>
        </Box>
      </Reveal>
      ...
      <Reveal when={sub >= 6}>
        <Box color={C.blue} style={{ width: "100%" }}>
          <T color={C.blue} bold center size={20}>Step 5: Hidden Layer Weight Gradients</T>
          <T color="#5eb3ff" size={16} style={{ marginTop: 10 }}>Now we are at the hidden layer weights. z₁ = x₁ x w₁₁ + x₂ x w₂₁ + b₁. So dz₁/dw₁₁ = x₁, dz₁/dw₂₁ = x₂, dz₁/db₁ = 1.</T>
          <div style={{ marginTop: 12, padding: "12px", borderRadius: 10, background: `${C.cyan}06`, border: `1px solid ${C.cyan}12` }}>
            <T color={C.cyan} bold size={16} center>Neuron h₁ weights (dLoss/dz₁ = -148.8)</T>
            <div style={{ marginTop: 10, display: "flex", flexDirection: "column", gap: 6 }}>
              {[
                ...
                <div key={name} style={{ display: "flex", alignItems: "center", gap: 0, padding: "8px 10px", borderRadius: 8, background: `${c}06`, border: `1px solid ${c}12` }}>
                  <code style={{ color: c, fontWeight: 800, fontSize: 15, padding: "4px 8px", background: `${c}12`, borderRadius: 5, textAlign: "center", minWidth: 80, flexShrink: 0 }}>dL/d{name}</code>
                  ...
                  <span style={{ color: C.dim, fontSize: 14, marginLeft: 6 }}>=</span>
                  <span style={{ padding: "4px 14px", marginLeft: 8, background: `${c}18`, borderRadius: 6, color: c, fontSize: 18, fontWeight: 800 }}>{result}</span>
                </div>
              ))}
            </div>
          </div>
          <div style={{ marginTop: 10, padding: "12px", borderRadius: 10, background: `${C.orange}06`, border: `1px solid ${C.orange}12` }}>
            <T color={C.orange} bold size={16} center>Neuron h₂ weights (dLoss/dz₂ = -37.2)</T>
            <div style={{ marginTop: 10, display: "flex", flexDirection: "column", gap: 6 }}>
              {[
          ...
          <T color="#5eb3ff" size={16} style={{ marginTop: 10 }}>Notice w₁₁ has a gradient of -223,200 while w₂₁ has only -446.4. Why? Because x₁ = 1500 is 500x larger than x₂ = 3. The gradient for a weight is proportional to its input - bigger inputs create bigger gradients.</T>
        </Box>
      </Reveal>
      ...
      <Reveal when={sub >= 7}>
        <Box color={C.orange} style={{ width: "100%" }}>
          <T color={C.orange} bold center size={20}>The Full Chain for w₁₁</T>
          <T color="#ffe082" size={16} style={{ marginTop: 10 }}>Let's trace the COMPLETE chain from loss all the way back to w₁₁. This is four multiplications - the chain rule in action:</T>
          <div style={{ marginTop: 12, padding: "14px", background: "rgba(0,0,0,0.4)", borderRadius: 12, border: `1px solid ${C.orange}25` }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 4, flexWrap: "wrap", marginBottom: 10 }}>
              ...
              <span style={{ padding: "2px 8px", background: `${C.red}15`, borderRadius: 4, color: "#ef9a9a", fontSize: 14, fontWeight: 600 }}>dLoss/dy</span>
              <span style={{ color: C.dim, fontSize: 14 }}>x</span>
              <span style={{ padding: "2px 8px", background: `${C.cyan}15`, borderRadius: 4, color: "#80deea", fontSize: 14, fontWeight: 600 }}>dy/dh₁</span>
              <span style={{ color: C.dim, fontSize: 14 }}>x</span>
              <span style={{ padding: "2px 8px", background: `${C.green}15`, borderRadius: 4, color: "#80e8a5", fontSize: 14, fontWeight: 600 }}>dReLU/dz₁</span>
              <span style={{ color: C.dim, fontSize: 14 }}>x</span>
              <span style={{ padding: "2px 8px", background: `${C.yellow}15`, borderRadius: 4, color: "#ffe082", fontSize: 14, fontWeight: 600 }}>dz₁/dw₁₁</span>
            </div>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 4, flexWrap: "wrap", marginBottom: 10 }}>
              <span style={{ color: C.dim, fontSize: 15 }}>=</span>
              <span style={{ padding: "2px 8px", background: `${C.red}15`, borderRadius: 4, color: "#ef9a9a", fontSize: 15, fontWeight: 600 }}>-186</span>
              <span style={{ color: C.dim, fontSize: 14 }}>x</span>
              <span style={{ padding: "2px 8px", background: `${C.cyan}15`, borderRadius: 4, color: "#80deea", fontSize: 15, fontWeight: 600 }}>0.8</span>
              <span style={{ color: C.dim, fontSize: 14 }}>x</span>
              <span style={{ padding: "2px 8px", background: `${C.green}15`, borderRadius: 4, color: "#80e8a5", fontSize: 15, fontWeight: 600 }}>1</span>
              <span style={{ color: C.dim, fontSize: 14 }}>x</span>
              <span style={{ padding: "2px 8px", background: `${C.yellow}15`, borderRadius: 4, color: "#ffe082", fontSize: 15, fontWeight: 600 }}>1500</span>
            </div>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
              <span style={{ color: C.dim, fontSize: 18 }}>=</span>
              <span style={{ padding: "3px 12px", background: `${C.red}15`, borderRadius: 4, color: C.red, fontSize: 20, fontWeight: 700 }}>-223,200</span>
            </div>
          </div>
          <T color="#ffe082" size={16} style={{ marginTop: 10 }}>Four multiplications. Each one traces through one operation in the computational graph. This is why it is called the "chain" rule - you chain derivatives together, link by link, all the way from loss back to each weight.</T>
        </Box>
      </Reveal>
      ...
      <Reveal when={sub >= 8}>
        <Box color={C.green} style={{ width: "100%" }}>
          <T color={C.green} bold center size={20}>Complete Gradient Table - Every Parameter</T>
          <T color="#80e8a5" size={16} style={{ marginTop: 10 }}>We just computed the gradient for ALL 9 learnable parameters in our network:</T>
          <div style={{ marginTop: 12, overflowX: "auto" }}>
            <div style={{ display: "grid", gridTemplateColumns: "auto auto auto auto", gap: 4, maxWidth: 400, margin: "0 auto" }}>
              {["Parameter", "Value", "Gradient", "Meaning"].map(h => (
                <T key={h} color={C.bright} bold size={13} center style={{ padding: "4px 6px", borderBottom: `1px solid ${C.green}30` }}>{h}</T>
              ))}
              {[
                ...
                <div key={p} style={{ display: "contents" }}>
                  <T color={C.cyan} bold size={13} center style={{ padding: "3px 6px" }}>{p}</T>
                  <T color={C.dim} size={13} center style={{ padding: "3px 6px" }}>{v}</T>
                  <T color={C.red} bold size={13} center style={{ padding: "3px 6px" }}>{g}</T>
                  <T color={C.green} size={12} center style={{ padding: "3px 6px" }}>{m}</T>
                </div>
              ))}
            </div>
          </div>
          <T color="#80e8a5" size={16} style={{ marginTop: 12 }}>Every gradient is negative. This means: increasing any weight will decrease the loss. That makes sense - the prediction (807) was too low, so the network needs to push everything up to get closer to 900.</T>
          <T color={C.bright} size={16} style={{ marginTop: 8 }}>This is backpropagation. Start at the loss, trace backward through every operation, multiply derivatives at each step. Every weight gets a gradient that tells it exactly how to change.</T>
        </Box>
      </Reveal>
...
export const GradientsInAction = (ctx) => {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 16 }}>
      ...
      {sub >= 0 && (
        <Box color={C.cyan} style={{ width: "100%" }}>
          <T color={C.cyan} bold center size={20}>We Have the Gradients - Now Use Them</T>
          <T color="#80deea" size={16} style={{ marginTop: 12 }}>Backprop gave us a gradient for every parameter. Now we apply gradient descent to ALL of them simultaneously. Same formula from chapter 1.12:</T>
          <div style={{ marginTop: 10, padding: "12px", background: "rgba(0,0,0,0.4)", borderRadius: 10, border: `1px solid ${C.green}25`, textAlign: "center" }}>
            <T color={C.green} bold size={18} style={{ fontFamily: "monospace" }}>new_weight = old_weight - learning_rate x gradient</T>
          </div>
          <T color="#80deea" size={16} style={{ marginTop: 10 }}>Learning rate = 0.0000001 (very small because our gradients are huge numbers like -223,200)</T>
        </Box>
      )}
      ...
      <Reveal when={sub >= 1}>
        <Box color={C.purple} style={{ width: "100%" }}>
          <T color={C.purple} bold center size={20}>Updating Every Weight at Once</T>
          <T color="#b8a9ff" size={16} style={{ marginTop: 10 }}>Learning rate = 0.0000001. Each weight gets nudged in the opposite direction of its gradient:</T>
          <div style={{ marginTop: 12, display: "flex", flexDirection: "column", gap: 5 }}>
            {[
              ...
              <div key={p} style={{ display: "flex", alignItems: "center", gap: 0, padding: "7px 10px", borderRadius: 8, background: `${c}06`, border: `1px solid ${c}12` }}>
                <code style={{ color: c, fontWeight: 800, fontSize: 15, padding: "3px 6px", background: `${c}12`, borderRadius: 5, textAlign: "center", minWidth: 52, flexShrink: 0 }}>{p}</code>
                <span style={{ color: C.dim, fontSize: 14, marginLeft: 8 }}>=</span>
                <span style={{ color: C.mid, fontSize: 14, marginLeft: 6 }}>{old}</span>
                <span style={{ color: C.dim, fontSize: 13, marginLeft: 6 }}>- lr ×</span>
                <span style={{ padding: "2px 8px", marginLeft: 4, background: big ? `${C.red}10` : "rgba(255,255,255,0.03)", borderRadius: 4, color: big ? "#ef9a9a" : C.dim, fontSize: 13, fontWeight: big ? 600 : 400 }}>{grad}</span>
                <span style={{ color: C.dim, fontSize: 14, marginLeft: 6 }}>=</span>
                <span style={{ padding: "4px 12px", marginLeft: 8, background: `${C.green}18`, borderRadius: 6, color: C.green, fontSize: 17, fontWeight: 800 }}>{nw}</span>
              </div>
            ))}
          </div>
          <T color="#b8a9ff" size={16} style={{ marginTop: 10 }}>Weights with large gradients (w₁₁, w_o1) changed the most. Biases barely moved because their gradients are small. All 9 parameters updated simultaneously - one step of training.</T>
        </Box>
      </Reveal>
      ...
      <Reveal when={sub >= 2}>
        <Box color={C.green} style={{ width: "100%" }}>
          <T color={C.green} bold center size={20}>Forward Pass with Updated Weights</T>
          <T color="#80e8a5" size={16} style={{ marginTop: 10 }}>Let's run the network again with the new weights and see if it improved:</T>
          <div style={{ marginTop: 12, display: "flex", flexDirection: "column", gap: 8 }}>
            <div style={{ padding: "8px 12px", borderRadius: 8, background: `${C.cyan}06`, border: `1px solid ${C.cyan}12` }}>
              <T color={C.cyan} bold size={15}>h₁ = ReLU(1500 x 0.522 + 3 x 50 + 50) = ReLU(983) = 983</T>
            </div>
            <div style={{ padding: "8px 12px", borderRadius: 8, background: `${C.orange}06`, border: `1px solid ${C.orange}12` }}>
              <T color={C.orange} bold size={15}>h₂ = ReLU(1500 x 0.106 + 3 x 10 + 5) = ReLU(194) = 194</T>
            </div>
            <div style={{ padding: "8px 12px", borderRadius: 8, background: `${C.green}06`, border: `1px solid ${C.green}12` }}>
              <T color={C.green} bold size={15}>y = 983 x 0.818 + 194 x 0.203 + 10 = 854</T>
            </div>
          </div>
          <div style={{ marginTop: 12, display: "flex", gap: 12, flexWrap: "wrap", justifyContent: "center" }}>
            <div style={{ padding: "10px 16px", background: `${C.red}08`, borderRadius: 8, border: `1px solid ${C.red}20`, textAlign: "center" }}>
              <T color={C.dim} size={12}>Before</T>
              <T color={C.red} bold size={16}>y = 807</T>
              <T color={C.red} bold size={14}>Loss = 8,649</T>
            </div>
            <div style={{ display: "flex", alignItems: "center" }}>
              <T color={C.bright} bold size={20}>→</T>
            </div>
            <div style={{ padding: "10px 16px", background: `${C.green}08`, borderRadius: 8, border: `1px solid ${C.green}20`, textAlign: "center" }}>
              <T color={C.dim} size={12}>After 1 step</T>
              <T color={C.green} bold size={16}>y = 854</T>
              <T color={C.green} bold size={14}>Loss = 2,116</T>
            </div>
          </div>
          <T color="#80e8a5" size={16} style={{ marginTop: 12 }}>Loss dropped from 8,649 to 2,116. Prediction moved from 807 to 854 - closer to the target of 900. One single step of backprop + gradient descent already cut the loss by 75%!</T>
        </Box>
      </Reveal>
      ...
      <Reveal when={sub >= 3}>
        <Box color={C.yellow} style={{ width: "100%" }}>
          <T color={C.yellow} bold center size={20}>Repeat - Watch the Network Converge</T>
          <T color="#ffe082" size={16} style={{ marginTop: 10 }}>Keep repeating: forward pass, compute loss, backprop, update weights. Each step gets closer:</T>
          <div style={{ marginTop: 12, display: "flex", flexDirection: "column", gap: 6 }}>
            {[
              ...
              <div key={step} style={{ display: "flex", alignItems: "center", gap: 8, padding: "4px 0" }}>
                <T color={C.dim} size={13} style={{ minWidth: 95, textAlign: "right" }}>{step}</T>
                <div style={{ flex: 1, height: 14, background: "rgba(255,255,255,0.04)", borderRadius: 4, overflow: "hidden" }}>
                  <div style={{ width: `${Math.max(bar, 0.5)}%`, height: "100%", background: c, borderRadius: 4, opacity: 0.6 }} />
                </div>
                <T color={c} bold size={13} style={{ minWidth: 50, textAlign: "right" }}>y={pred}</T>
                <T color={C.dim} size={12} style={{ minWidth: 65, textAlign: "right" }}>loss={loss}</T>
              </div>
            ))}
          </div>
          <T color="#ffe082" size={16} style={{ marginTop: 12 }}>After just 10 steps, the network predicts 899.8 - nearly perfect. Loss went from 8,649 to 0.04. This is gradient descent: small, calculated steps downhill until you reach the minimum.</T>
        </Box>
      </Reveal>
      ...
      <Reveal when={sub >= 4}>
        <Box color={C.blue} style={{ width: "100%" }}>
          <T color={C.blue} bold center size={20}>The Complete Training Loop</T>
          <T color="#5eb3ff" size={16} style={{ marginTop: 10 }}>Every neural network in the world trains using this exact loop:</T>
          <div style={{ marginTop: 12, display: "flex", flexDirection: "column", gap: 4 }}>
            {[
              { step: "1", label: "Forward Pass", desc: "Feed input through network, get prediction", c: C.cyan, icon: "→" },
              { step: "2", label: "Compute Loss", desc: "How wrong was the prediction?", c: C.red, icon: "?" },
              { step: "3", label: "Backward Pass", desc: "Chain rule: compute gradient for every weight", c: C.purple, icon: "←" },
              { step: "4", label: "Update Weights", desc: "Nudge each weight opposite to its gradient", c: C.green, icon: "↓" },
              { step: "↻", label: "Repeat", desc: "Thousands to billions of times", c: C.yellow, icon: "↻" },
            ].map(({ step, label, desc, c, icon }) => (
              <div key={step} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 12px", borderRadius: 8, background: `${c}06`, border: `1px solid ${c}12` }}>
                <div style={{ minWidth: 36, height: 36, borderRadius: "50%", background: `${c}20`, display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, color: c, fontSize: 16, flexShrink: 0 }}>{icon}</div>
                <div>
                  <T color={c} bold size={16}>{label}</T>
                  <T color={C.dim} size={14}>{desc}</T>
                </div>
              </div>
            ))}
          </div>
          <T color="#5eb3ff" size={16} style={{ marginTop: 12 }}>GPT-4 runs this loop trillions of times on billions of text examples. Each loop adjusts millions of weights by tiny amounts. After enough loops, the model can write poetry, answer questions, and write code - all because of this simple loop: predict, measure error, trace blame, fix weights, repeat.</T>
        </Box>
      </Reveal>
...
export const WhyBackpropHard = (ctx) => {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 16 }}>
      ...
      {sub >= 0 && (
        <Box color={C.cyan} style={{ width: "100%" }}>
          <T color={C.cyan} bold center size={20}>Our 2-Layer Network Was the Easy Case</T>
          <T color="#80deea" size={16} style={{ marginTop: 12 }}>For w₁₁, the chain had 4 multiplications: dLoss/dy x dy/dh₁ x dReLU/dz₁ x dz₁/dw₁₁. That is manageable.</T>
          <T color="#80deea" size={16} style={{ marginTop: 8 }}>But modern networks have 50, 100, or even 1000+ layers. GPT-4 has 120 layers. For a weight in layer 1 of a 120-layer network, the chain rule has 120+ multiplications.</T>
          <div style={{ marginTop: 14, display: "flex", alignItems: "center", justifyContent: "center", gap: 6, flexWrap: "wrap" }}>
            {["Layer 1", "Layer 2", "Layer 3", "Layer 4", "Layer 5"].map((label, i) => (
              <div key={i} style={{ display: "flex", alignItems: "center", gap: 6 }}>
                <div style={{ padding: "6px 12px", borderRadius: 6, background: `${C.cyan}10`, border: `1px solid ${C.cyan}30`, textAlign: "center" }}>
                  <T color={C.cyan} bold size={13}>{label}</T>
                </div>
                {i < 4 && <T color={C.dim} size={14}>×</T>}
              </div>
            ))}
            <T color={C.dim} size={14} style={{ margin: "0 4px" }}>... ×</T>
            <div style={{ padding: "6px 12px", borderRadius: 6, background: `${C.red}10`, border: `1px solid ${C.red}30`, textAlign: "center" }}>
              <T color={C.red} bold size={13}>Loss</T>
            </div>
          </div>
          <T color={C.dim} size={13} center style={{ marginTop: 8 }}>120 multiplications chained together</T>
        </Box>
      )}
      ...
      <Reveal when={sub >= 1}>
        <Box color={C.red} style={{ width: "100%" }}>
          <T color={C.red} bold center size={20}>The Vanishing Gradient Problem</T>
          <T color="#ef9a9a" size={16} style={{ marginTop: 10 }}>If each derivative in the chain is less than 1, the product shrinks exponentially:</T>
          <div style={{ marginTop: 12, display: "flex", flexDirection: "column", gap: 4 }}>
            {[
              ...
              <div key={layers} style={{ display: "flex", alignItems: "center", gap: 8, padding: "4px 8px" }}>
                ...
                <div style={{ flex: 1, height: 12, background: "rgba(255,255,255,0.04)", borderRadius: 4, overflow: "hidden" }}>
                  <div style={{ width: `${Math.max(bar, 0.3)}%`, height: "100%", background: C.red, borderRadius: 4, opacity: 0.7 }} />
                </div>
                <T color={C.red} bold size={12} style={{ minWidth: 120, textAlign: "right" }}>{result}</T>
              </div>
            ))}
          </div>
          <T color="#ef9a9a" size={16} style={{ marginTop: 10 }}>At 50 layers, the gradient is essentially zero. The early layers never learn because the gradient signal has completely vanished by the time it reaches them. It is like whispering a message through 50 people - by the end, nothing is left.</T>
        </Box>
      </Reveal>
      ...
      <Reveal when={sub >= 2}>
        <Box color={C.orange} style={{ width: "100%" }}>
          <T color={C.orange} bold center size={20}>The Exploding Gradient Problem</T>
          <T color="#ffe082" size={16} style={{ marginTop: 10 }}>The opposite problem: if each derivative is greater than 1, the product grows exponentially:</T>
          <div style={{ marginTop: 12, display: "flex", flexDirection: "column", gap: 4 }}>
            {[
              ...
              <div key={layers} style={{ display: "flex", alignItems: "center", gap: 8, padding: "4px 8px" }}>
                <T color={C.dim} size={13} style={{ minWidth: 75, textAlign: "right" }}>{layers}</T>
                <div style={{ flex: 1, height: 12, background: "rgba(255,255,255,0.04)", borderRadius: 4, overflow: "hidden" }}>
                  <div style={{ width: `${Math.min(bar, 100)}%`, height: "100%", background: C.orange, borderRadius: 4, opacity: 0.7 }} />
                </div>
                <T color={C.orange} bold size={12} style={{ minWidth: 140, textAlign: "right" }}>{result}</T>
              </div>
            ))}
          </div>
          <T color="#ffe082" size={16} style={{ marginTop: 10 }}>At 50 layers, the gradient is astronomically large. Weight updates become wild jumps that overshoot the minimum and the network diverges - the loss goes to infinity instead of zero. Training crashes.</T>
        </Box>
      </Reveal>
      ...
      <Reveal when={sub >= 3}>
        <Box color={C.green} style={{ width: "100%" }}>
          <T color={C.green} bold center size={20}>Solutions That Made Deep Learning Work</T>
          <T color="#80e8a5" size={16} style={{ marginTop: 10 }}>These problems held back AI for decades. Here is what finally solved them:</T>
          <div style={{ marginTop: 12, display: "flex", flexDirection: "column", gap: 10 }}>
            <div style={{ padding: "10px 12px", borderRadius: 8, background: `${C.green}06`, border: `1px solid ${C.green}12` }}>
              <T color={C.green} bold size={16}>ReLU Activation (instead of sigmoid)</T>
              <T color={C.dim} size={14} style={{ marginTop: 4 }}>Sigmoid's derivative is at most 0.25 - guaranteed to vanish. ReLU's derivative is exactly 1 for positive inputs - no shrinkage. This alone was a breakthrough.</T>
            </div>
            <div style={{ padding: "10px 12px", borderRadius: 8, background: `${C.yellow}06`, border: `1px solid ${C.yellow}12` }}>
              <T color={C.yellow} bold size={16}>Residual Connections (Skip Connections)</T>
              <T color={C.dim} size={14} style={{ marginTop: 4 }}>Instead of output = f(input), use output = input + f(input). The gradient for "input" is always 1 - it flows straight through the skip connection, no matter how deep the network. The Transformer uses this everywhere.</T>
            </div>
            <div style={{ padding: "10px 12px", borderRadius: 8, background: `${C.purple}06`, border: `1px solid ${C.purple}12` }}>
              <T color={C.purple} bold size={16}>Layer Normalization</T>
              <T color={C.dim} size={14} style={{ marginTop: 4 }}>Keep activations in a stable range at every layer. Prevents both tiny and huge values from accumulating through the network. We will see this in Section 8.</T>
            </div>
            <div style={{ padding: "10px 12px", borderRadius: 8, background: `${C.cyan}06`, border: `1px solid ${C.cyan}12` }}>
              <T color={C.cyan} bold size={16}>Careful Weight Initialization</T>
              <T color={C.dim} size={14} style={{ marginTop: 4 }}>Start weights in a range that keeps gradients close to 1. Too large = explosion. Too small = vanishing. Methods like "He initialization" and "Xavier initialization" solve this mathematically.</T>
            </div>
          </div>
        </Box>
      </Reveal>
      ...
      <Reveal when={sub >= 4}>
        <Box color={C.blue} style={{ width: "100%" }}>
          <T color={C.blue} bold center size={20}>Why This Matters for Everything Ahead</T>
          <T color="#5eb3ff" size={16} style={{ marginTop: 10 }}>Every architecture choice in the Transformer - residual connections, layer normalization, scaled dot-product attention - exists partly to solve these gradient problems. When you see "Add & Norm" in the Transformer diagram, you will know exactly why it is there: to keep gradients healthy so backprop works through 120 layers.</T>
          <div style={{ marginTop: 12, padding: "12px", background: "rgba(0,0,0,0.4)", borderRadius: 10, border: `1px solid ${C.blue}25` }}>
            <T color={C.bright} size={16} center>Backpropagation is the single most important algorithm in deep learning. Every model you have ever used - GPT, DALL-E, AlphaGo - learned through this exact process: forward pass, compute loss, chain rule backward, update weights, repeat.</T>
          </div>
        </Box>
      </Reveal>
      ...
      {sub >= 0 && (
        <Box color={C.cyan} style={{ width: "100%" }}>
          ...
          <div style={{ marginTop: 20, display: "flex", flexDirection: "column", alignItems: "center", gap: 10 }}>
            ...
            <div style={{ display: "flex", gap: 20, alignItems: "center", justifyContent: "center" }}>
              <div style={{ width: 80, height: 80, background: `${C.cyan}15`, border: `1px solid ${C.cyan}30`, display: "flex", alignItems: "center", justifyContent: "center", borderRadius: 8 }}>
                <T size={24} bold color={C.cyan}>42</T>
              </div>
              <T size={28} color={C.mid}>→</T>
              <div style={{ display: "flex", gap: 6 }}>
                {[3, 7, 2, 5].map((num, i) => (
                  <div key={i} style={{ width: 70, height: 70, background: `${[C.cyan, C.green, C.yellow, C.orange][i]}15`, border: `1px solid ${[C.cyan, C.green, C.yellow, C.orange][i]}30`, display: "flex", alignItems: "center", justifyContent: "center", borderRadius: 8 }}>
