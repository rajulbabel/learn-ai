      ...
      <Reveal when={sub >= 5}>
        <Box color={C.pink} style={{ width: "100%" }}>
          ...
          <T color="#ce93d8" style={{ marginTop: 8 }}>
            People were training models with way too few tokens relative to size.
          </T>
          <T color="#ce93d8" style={{ marginTop: 6 }}>
            <strong>Most models are under-trained, not too small.</strong>
          </T>
          <T color="#ce93d8" style={{ marginTop: 6 }}>
            Optimal: match data tokens to parameters. Roughly 20 tokens per parameter, so 200B params needs about 4T
            tokens.
          </T>
        </Box>
      </Reveal>
...
export const ParametersAtScale = (ctx) => {
  const { sub, subBtnRipple, setSubBtnRipple, registerSubBtn, navigate } = ctx;
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 16 }}>
      {sub >= 0 && (
...
export const TrainingPipeline = (ctx) => {
  const { sub, subBtnRipple, setSubBtnRipple, registerSubBtn, navigate } = ctx;
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 16 }}>
      {sub >= 0 && (
